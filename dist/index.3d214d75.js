...
var _lookPathDefault = parcelHelpers.interopDefault(_lookPath);
var _movePathList = require("./js/movePathList");
var _movePathListDefault = parcelHelpers.interopDefault(_movePathList);
...
var _createEnemyDefault = parcelHelpers.interopDefault(_createEnemy);
var _createShip = require("./js/createShip");
var _createShipDefault = parcelHelpers.interopDefault(_createShip);
...
function keyCodeMap(keycode, type) {
    ...
    else ship.skill(keycode);
}
function actionMove(ship1) {
    var action = {
        UP: function() {
            var ps = ship1.position;
            ship1.position = ps - (0, _config.w) > -1 ? ps - (0, _config.w) : ps;
        },
        RIGHT: function() {
            var ps = ship1.position;
            if (ps + 1 <= (0, _config.w) * (0, _config.h) - 1) {
                if ((ps + 1) % (0, _config.w) !== 0) ship1.position = ps + 1;
            }
        },
        DOWN: function() {
            var ps = ship1.position;
            var nowY = Math.round((ps - ps % (0, _config.w)) / (0, _config.w));
            if (ps + (0, _config.w) < (0, _config.w) * (0, _config.h)) ship1.position = nowY < (0, _config.h) ? ps + (0, _config.w) : ps;
        },
        LEFT: function() {
            var ps = ship1.position;
            if (ps - 1 > -1) {
                if (ps % (0, _config.w) !== 0) ship1.position = ps - 1;
            }
        },
        SPACE: function() {
            shotDriver();
    ...
    for(var key in keyType)if (keyType[key] == true) action[key]();
    if (!keyType.SPACE) closeShot();
}
function shotDriver() {
    if (!shotFn) shotFn = setInterval(function() {
        ship.shot();
        ship.lookType = "OPEN";
    }, 1000 / ship.shotFps);
}
...
function bulletPosition(shipPs) {
    var enemyBulleArr = (0, _config.renderData).enemyBullet;
...
function clearRenderData() {
    (0, _config.renderData).enemy = [];
    (0, _config.renderData).boss = [];
    // renderData.enemyBullet = [];
    // renderData.aniEffect = [];
    (0, _config.renderData).item = [];
    (0, _config.renderData).skills = [];
//console.log(renderData);
}
var ship = new (0, _createShipDefault.default)({
    name: "CrystalShip",
    life: (0, _config.shipLife),
    position: (0, _config.w) * Math.floor((0, _config.h) / 2) - Math.floor((0, _config.w) / 2),
    deadPosition: (0, _config.w) * Math.floor((0, _config.h) / 2) - Math.floor((0, _config.w) / 2),
    look: "crystal",
    skills: [
        {
            skillKey: 65,
...
function gaphic(TYPE) {
    ...
    (0, _scriptDefault.default)(scriptMileage, ruleObj, nextPolling);
    if (TYPE === "OBJ_MOVE") {
        var createObj = false;
        ...
        if (renCount == nextPolling && !stopEnemyPush) {
            for(var x = 0; x < ruleObj.enemyQuantity; x++){
                ...
                if (enemyObj) {
                    enemyObj.position = Math.floor(Math.random() * (0, _config.w));
                    enemyObj.deadCb = function() {
                        killCount++;
                        if (bestScore < killCount) localStorage.setItem("bestScore", killCount);
                    };
                    (0, _config.renderData).enemy.push(new (0, _createEnemyDefault.default)(enemyObj));
                }
            }
            if (ruleObj.enemyPolling) nextPolling = renCount + Math.floor(Math.random() * ruleObj.enemyPolling[1] + ruleObj.enemyPolling[0]);
        }
        if (nextPolling < renCount && ruleObj.enemyPolling) nextPolling = renCount + Math.floor(Math.random() * ruleObj.enemyPolling[1] + ruleObj.enemyPolling[0]);
        // boss push
        if (boss.length > 0) {
            var defPosition = Math.floor(Math.random() * (0, _config.w));
        ...
        if (item) {
            item.forEach(function(itemData) {
                itemData.position = Math.floor(Math.random() * (0, _config.w));
                (0, _config.renderData).item.push(new (0, _createItemDefault.default)(itemData));
            });
            ruleObj.item = null;
        }
        bgCount++;
    ...
    if (TYPE === "BULLET_MOVE") bulletPosition(ship.position);
    //canvas
    var viewDom = document.getElementById("view").getContext("2d");
    viewDom.clearRect(0, 0, (0, _config.vwidth), (0, _config.vheight));
    // ship
    ship.grapic(viewDom);
    //enmyBullet
    (0, _config.renderData).enemyBullet.map(function(bullt) {
        var enemyImg = document.getElementById(bullt.data.look);
        viewDom.drawImage(enemyImg, bullt.data.x - 7.5, bullt.data.y - 5, bullt.data.w, bullt.data.h);
    });
    // enmy
    (0, _config.renderData).enemy.forEach(function(obj) {
        obj.action(TYPE, viewDom, ship);
    });
    // skill
    (0, _config.renderData).skills.forEach(function(skill, index) {
        skill(ship, viewDom) && (0, _config.renderData).skills.splice(index, 1);
    });
    // shipskill
    (0, _config.renderData).shipSkills.forEach(function(skill, index) {
        skill(ship, viewDom) && (0, _config.renderData).shipSkills.splice(index, 1);
    });
    // item
    (0, _config.renderData).item.forEach(function(obj) {
        obj.action(TYPE, viewDom, ship.position);
    });
    // effect
    (0, _config.renderData).aniEffect.forEach(function(el) {
        el(viewDom);
    });
    //bg
    document.getElementById("view").style.backgroundPositionY = bgCount + "px";
    document.getElementById("score").innerHTML = 'Score: <div class="score">' + killCount + "</div><br/> Mileage: " + scriptMileage + "<br/>Best score: " + (localStorage.getItem("bestScore") || 0) + "<br/> Best Mileage: " + bestMileage + "<br/> Life: " + ship.life + "/" + ship.maxLife;
...
function touchAction(event) {
    ...
    if (event.touches[0]) {
        var x = Math.floor(event.touches[0].pageX / (0, _config.pixelWeigth));
        var y = Math.floor(event.touches[0].pageY / (0, _config.pixelWeigth));
        var _ps = (0, _config.w) * y + x;
        ship.position = _ps;
        shotDriver();
    }
}
document.getElementById("view").height = (0, _config.vheight);
document.getElementById("view").width = (0, _config.vwidth);
var preTimetamp = null;
function step(timestamp) {
    var progress;
    if (preTimetamp === null) preTimetamp = timestamp;
    progress = timestamp - preTimetamp;
    if (progress >= (0, _config.renderTime)) {
        preTimetamp = timestamp;
        //animation
        gaphic("OBJ_MOVE");
        gaphic("CONTROL_MOVE");
        !(0, _config.atc).isMobile() && actionMove(ship);
        gaphic("BULLET_MOVE");
    //
    }
...
window.r = (0, _config.renderData); //todo
 // 來一點速度感的特效
 //*把敵人也改成可以用其他磚塊拼
 // 加入盾牌道具
 // 加入近戰道具
...
function circle(x, y, r, all, now, margin) {
    ...
    return {
        ...
        outScope: _x < 0 || _x > (0, _config.w) - 1 || _y < 0 || _y > (0, _config.h) - 1
    };
}
function circleMap() {
    var renCount = 0;
...
exports.default = {
    ...
    "MONSTER1_openmouth": createMapfn([
        [
            -3,
            2
        ],
...
},{"./config":"4Wc5b","./positionMethod":"gqbqK","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"4Wc5b":[function(require,module,exports) {
var atc = {
    isMobile: function() {
        var e = !1;
        return function(n) {
...
},{}],"gqbqK":[function(require,module,exports) {
var _config = require("./config");
var positionTosXsY = function(position) {
    var x = position % (0, _config.w);
    var y = Math.round((position - x) / (0, _config.w));
    return {
        x: x,
        y: y
    };
...
exports.positionToXY = function(ps) {
    var x = ps % (0, _config.w);
    var y = Math.round((ps - x) / (0, _config.w));
    return {
        ps: ps,
        x: x * (0, _config.pixelWeigth) + (0, _config.pixelWeigth) / 2,
        y: y * window.innerHeight / (0, _config.h),
        __x: x,
        __y: y,
        limit: (0, _config.w) * (0, _config.h) - 1
    };
};
...
// }
exports.xyToPosition = function(x, y) {
    var _x = Math.round(x / (0, _config.pixelWeigth));
    var _y = Math.round(y / (0, _config.pixelWeigth));
    return _y * (0, _config.w) + _x;
};
exports.positionTosXsY = positionTosXsY;
exports.sXsYToPosition = function(sx, sy) {
    return sy * (0, _config.w) + sx;
};
exports.ezPosition = function(position) {
    var ps = position;
    return function(x, y, clear) {
        var rs = ps + y * (0, _config.w) + x;
        if (clear) ps = null;
        return rs;
    };
};
exports.ezPositionWithCheckScope = function(position) {
    ...
    return function(x, y, clear) {
        ...
        return _x < 0 || _x > (0, _config.w) - 1 || _y < 0 || _y > (0, _config.h) - 1 ? undefined : rs;
    };
};
exports.touchScope = function(position) {
    ...
    return {
        TOP: ps.y === 0,
        RIGHT: ps.x === (0, _config.w) - 1,
        BOTTOM: ps.y === (0, _config.h) - 1,
        LEFT: ps.x === 0
    };
};
...
},{"./config":"4Wc5b"}],"gkKU3":[function(require,module,exports) {
...
},{}],"bWHoT":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _config = require("./config");
var _positionMethod = require("./positionMethod");
function positionLimit(nowPosition) {
    var leftlimt = this.mainX - this.mainX % (0, _config.w);
    var rightlimt = leftlimt + (0, _config.w) - 1;
    ...
    this.mainX += (0, _config.w);
    return nowPosition;
}
exports.default = {
    goStop: function() {
        var me = this;
        if (this.getST() < 6) this.position += (0, _config.w);
        if (this.getST() > 80) this.position += (0, _config.w);
        if (this.position > (0, _config.w) * (0, _config.h) - 1) (0, _config.renderData).enemy.find(function(el, index) {
            if (el === me) (0, _config.renderData).enemy.splice(index, 1);
        });
    },
    gostMove: function() {
        var me = this;
        var thisY = Math.floor(this.position / (0, _config.h));
        var r = (0, _config.h) / 4;
        var nowY = Math.floor(this.mainX / (0, _config.w));
        ...
        while(this.position == -1)this.position = positionLimit.call(this, this.mainX + xMargin);
        if (this.position > (0, _config.w) * (0, _config.h) - 1) (0, _config.renderData).enemy.find(function(el, index) {
            if (el === me) (0, _config.renderData).enemy.splice(index, 1);
        });
    },
    goToOut: function() {
        this.position += (0, _config.w);
        var me = this;
        if (this.position > (0, _config.w) * (0, _config.h) - 1) (0, _config.renderData).enemy.find(function(el, index) {
            if (el === me) (0, _config.renderData).enemy.splice(index, 1);
        });
...
},{"./config":"4Wc5b","./positionMethod":"gqbqK","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"4pp4s":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _config = require("./config");
...
var GOZILLA = {
    ...
    position: Math.round((0, _config.w) / 3) - 5,
    bulletType: "track",
    skills: [
        {
            type: "atomicExplosion",
...
var GOZILLA2 = {
    ...
    look: "MONSTER1",
    shotLook: "MONSTER1_openmouth",
    position: Math.round((0, _config.w) / 3) * 2 - 5,
    bulletType: "track",
    skills: [
        {
            type: "atomicExplosion",
...
var GreenPoint = {
    ...
    effect: function(ship) {
        (0, _config.renderData).aniEffect.push((0, _aniEffectMethod.animation)(80, function(renCount, viewDom) {
...
exports.default = function(renCount, ruleObj, nextPolling) {
    ...
    if (rule) Object.keys(rule).forEach(function(key) {
        ruleObj[key] = rule[key];
    });
};

},{"./config":"4Wc5b","./createEnemy":"7V1mZ","./aniEffectMethod":"k9PxR","./positionMethod":"gqbqK","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"7V1mZ":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
var _enemyBulletTypeDefault = parcelHelpers.interopDefault(_enemyBulletType);
var _enemySkillType = require("./enemySkillType");
var _enemySkillTypeDefault = parcelHelpers.interopDefault(_enemySkillType);
var _bricks = require("./bricks");
var _bricksDefault = parcelHelpers.interopDefault(_bricks);
var _sound = require("./sound");
var _config = require("./config");
function enemyMove() {
    if ((0, _movePathListDefault.default)[this.movePath] && this.getST() % this.moveTime == 0) (0, _movePathListDefault.default)[this.movePath].call(this);
}
function shotByEnemy(position, bulletType, ship) {
    //var bulletArr = renderData.enemyBullet;
    //var dataFn = 
    (0, _enemyBulletTypeDefault.default)[bulletType](position, ship);
...
function launchSkillsByEnemy(position, Type, ship) {
    ...
    var skillArr = (0, _config.renderData).skills.push((0, _enemySkillTypeDefault.default)[Type](position, ship));
...
exports.default = function(obj) {
    ...
    this.lifeBarPosition = obj.lifeBarPosition || function(position) {
        return position;
    };
    this.position = obj.position; //位置
    this.shot = obj.shot; //會不會發射子彈
    this.shotTime = obj.shotTime; //連發數
    this.shotPoint = obj.shotPoint || function(position) {
        return [
            position
        ];
    }; //發射點
    this.skillPoint = obj.skillPoint || function(position) {
        return [
            position
        ];
    }; //技能發射點
    ...
    this.wasHit = function(bulletPs, viewDom, cb) {
        var me = this;
        var path = (0, _lookPathDefault.default)[this.look](this.position).map((el)=>el.ps);
        if (path.includes(bulletPs)) {
            hit = true;
            this.life--;
            if (this.life < 1) {
                (0, _config.renderData).enemy.find(function(el, index) {
                    if (el === me) {
                        var bestScore = localStorage.getItem("bestScore");
                        (0, _config.renderData).enemy.splice(index, 1);
                    }
                });
                this.dead();
            } else {
    ...
    this.grapic = function(viewDom, lootype) {
        ...
        (0, _lookPathDefault.default)[this.look](this.position).forEach(function(ps, index) {
            (0, _bricksDefault.default)(ps, viewDom, color);
        });
        if (this.showLife) {
            var ps1 = (0, _positionMethod.positionToXY)(this.lifeBarPosition(this.position));
            viewDom.fillStyle = "rgba(255,255,255,.3)";
            viewDom.fillRect(ps1.x - 50, ps1.y, 100, 5);
            viewDom.fillStyle = "orange";
            viewDom.fillRect(ps1.x - 50, ps1.y, Math.round(100 * this.life / this.maxlife), 5);
        }
    };
        ...
        if (renderType === "OBJ_MOVE") {
            ...
            this.skills.forEach((function(skill) {
                if (survivalTime % skill.launchTime[0] < skill.launchTime[1]) {
                    ...
                    if (launchCount % skill.launchTime[2] == 0) skill.skillPoint(this.position).forEach(function(position) {
            ...
            if (survivalTime % this.shotTime[0] < this.shotTime[1] && !isLaunchSkill) {
                ...
                if (shotCount % this.shotTime[2] == 0) this.shotPoint(this.position).forEach((function(position) {
                    if (this.shot) {
    ...
    this.dead = function() {
        ...
        (0, _config.renderData).aniEffect.push((0, _aniEffectMethod.animation)(50, function(renCount, viewDom) {
            viewDom.drawImage(explosion, deadPs.x - (50 - renCount) / 2, deadPs.y, 120 - renCount, 120 - renCount);
        }));
        obj = null;
        (0, _sound.explosionSound)();
        deadCb();
    };
};

},{"./lookPath":"zDHOl","./movePathList":"bWHoT","./aniEffectMethod":"k9PxR","./positionMethod":"gqbqK","./enemyBulletType":"dB13B","./enemySkillType":"bmp9X","./bricks":"fh8xc","./sound":"kdkJ4","./config":"4Wc5b","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"k9PxR":[function(require,module,exports) {
var _config = require("./config");
exports.animation = function(totalRenCount, aniFn) {
    var renCount = 1;
    var fn = function(viewDom) {
        if (renCount == totalRenCount) (0, _config.renderData).aniEffect.splice((0, _config.renderData).aniEffect.indexOf(fn), 1);
...
},{"./config":"4Wc5b"}],"dB13B":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _config = require("./config");
var _positionMethod = require("./positionMethod");
var _aniEffectMethod = require("./aniEffectMethod");
var bulletArr = (0, _config.renderData).enemyBullet;
function bulletPush(data, bulletPs) {
    if (!bulletArr.reduce(function(rs, el) {
        return rs || el.data.position === bulletPs;
    }, false)) bulletArr.push(data);
}
...
exports.default = {
    normal: function(bulletPs, ship) {
        ...
        var dataFn = function(taPs, clear) {
            ...
            var p1 = (0, _positionMethod.positionTosXsY)(prePs + (0, _config.w) * (0, _config.h));
            ...
            var _p1 = (0, _positionMethod.positionToXY)(prePs + (0, _config.w) * (0, _config.h));
            var t = 1 / time * (time - count);
    ...
    track: function(bulletPs, ship) {
        ...
        var dataFn = function(taPs, clear) {
            ...
            var y = Math.round(p0.y + (p1.y - p0.y) * t);
            var _p0 = (0, _positionMethod.positionToXY)(prePs);
            var marginX = (0, _config.pixelWeigth) / 2;
            var marginY = 0;
            ...
            return {
                ...
                w: 15,
                h: 15,
                clear: count < -15 || x < 0 || x > (0, _config.w) - 1
            };
        };
        bulletPush({
            data: dataFn(ship.position),
    ...
    diffusionTrack: function(bulletPs, ship) {
        for(var i1 = 0; i1 < 3; i1++)(function(i) {
            //console.log('i',i);
            var prePs = bulletPs;
            var p0 = (0, _positionMethod.positionToXY)(prePs);
            var p2 = (0, _positionMethod.positionToXY)(ship.position);
            var distance = Math.floor(Math.sqrt((p0.x - p2.x) * (p0.x - p2.x) + (p0.y - p2.y) * (p0.y - p2.y)) * 2);
            var time = distance * .08;
            var p1 = (0, _aniEffectMethod.circle)(p0.x, p0.y, distance * .3, 16, i + 1, getAngle(p0, p2) * Math.PI / 180 + Math.PI / 180 * 45);
            var count = time;
            var dataFn = function(taPs, clear) {
                ...
                return {
                    position: (0, _positionMethod.xyToPosition)(x, y),
                    x: x,
                    y: y,
                    look: "bulletBall",
                    w: 15,
                    h: 15,
                    clear: count < -5 || (0, _positionMethod.xyToPosition)(x, y).x < 0 || (0, _positionMethod.xyToPosition)(x, y).x > (0, _config.w) - 1
                };
            };
            bulletPush({
                data: dataFn(ship.position),
...
},{"./config":"4Wc5b","./positionMethod":"gqbqK","./aniEffectMethod":"k9PxR","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"bmp9X":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _config = require("./config");
var _positionMethod = require("./positionMethod");
var _skillPath = require("./skillPath");
var _skillPathDefault = parcelHelpers.interopDefault(_skillPath);
var _bricks = require("./bricks");
var _bricksDefault = parcelHelpers.interopDefault(_bricks);
var _aniEffectMethod = require("./aniEffectMethod");
function circle(x, y, r, all, now, margin) {
    ...
    var _y = Math.round(y + r * Math.sin(ang * now + _margin));
    return {
        position_big: (0, _positionMethod.xyToPosition)(_x, _y),
        position: (0, _positionMethod.sXsYToPosition)(_x, _y),
        outScope: Math.round(_x / (0, _config.pixelWeigth)) < 0 || Math.round(_x / (0, _config.pixelWeigth)) > (0, _config.w) - 1 || Math.round(_y / (0, _config.pixelWeigth)) < 0 || Math.round(_y / (0, _config.pixelWeigth)) > (0, _config.h) - 1,
        x: _x,
        y: _y
    };
}
exports.default = {
    claw: function(position, ship1) {
        var time = 60;
        var prePs = position;
        var count = time;
        var _taPs = ship1.position;
        return function(ship, viewDom) {
            count--;
            var t = 1 / time * (time - count);
            var isHurt = false;
            if (count > time * .9) {
                (0, _skillPathDefault.default)["claw1"](_taPs).forEach(function(ps) {
                    (0, _bricksDefault.default)(ps, viewDom, "rgba(255,255,255,.8)");
                });
                return false;
            }
            if (count > time * .8) {
                (0, _skillPathDefault.default)["claw2"](_taPs).forEach(function(ps) {
                    (0, _bricksDefault.default)(ps, viewDom, "rgba(255,255,255,.2)");
                });
                return false;
            }
            if (count > time * .6) {
                (0, _skillPathDefault.default)["claw3"](_taPs).forEach(function(ps) {
                    (0, _bricksDefault.default)(ps, viewDom, "rgba(255,255,255,.6)");
            ...
            if (count > time * .4) {
                (0, _skillPathDefault.default)["claw4"](_taPs).forEach(function(ps) {
                    (0, _bricksDefault.default)(ps, viewDom, "rgba(255,255,255,.6)");
                    isHurt = isHurt || ship.path.includes(ps.ps);
                });
                if (isHurt) ship.injured();
            ...
            if (count > 0) {
                (0, _skillPathDefault.default)["claw5"](_taPs).forEach(function(ps) {
                    (0, _bricksDefault.default)(ps, viewDom, "rgba(255,255,255,1.0)");
                    isHurt = isHurt || ship.path.includes(ps.ps);
                });
                if (isHurt) ship.injured();
                return false;
    ...
    atomicExplosion: function(position, ship2) {
        var time = 300 - (40 - (0, _config.w)) * 20;
        var prePs = position;
        var count = time;
        var _taPs = ship2.position;
        return function(ship, viewDom) {
            count--;
            var t = 1 / time * (time - count);
            var isHurt = false;
            ...
            for(var i = 0; i <= pointLength; i++){
                ...
                !psData.outScope && skillPath.push({
...
},{"./config":"4Wc5b","./positionMethod":"gqbqK","./skillPath":"lttCW","./bricks":"fh8xc","./aniEffectMethod":"k9PxR","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"lttCW":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _config = require("./config");
var _positionMethod = require("./positionMethod");
...
},{"./config":"4Wc5b","./positionMethod":"gqbqK","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"fh8xc":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _positionMethod = require("./positionMethod");
var _config = require("./config");
var brickMap = {
    0: function(ps, viewDom, color) {
        ...
        viewDom.beginPath();
        viewDom.rect(psObj.x - (0, _config.pixelWeigth) / 2, psObj.y, (0, _config.pixelWeigth), (0, _config.pixelWeigth));
        viewDom.fillStyle = color || "white";
        viewDom.fill();
    },
    1: function(ps, viewDom, color) {
        var psObj = (0, _positionMethod.positionToXY)(ps);
        var defX = psObj.x - (0, _config.pixelWeigth) / 2;
        viewDom.beginPath();
        viewDom.moveTo(defX, psObj.y);
        viewDom.lineTo(defX, psObj.y + (0, _config.pixelWeigth));
        viewDom.lineTo(defX + (0, _config.pixelWeigth), psObj.y + (0, _config.pixelWeigth));
        viewDom.fillStyle = color || "white";
        viewDom.fill();
    },
    2: function(ps, viewDom, color) {
        ...
        viewDom.beginPath();
        viewDom.moveTo(defX, psObj.y);
        viewDom.lineTo(defX + (0, _config.pixelWeigth), psObj.y);
        viewDom.lineTo(defX + (0, _config.pixelWeigth), psObj.y + (0, _config.pixelWeigth));
        viewDom.fillStyle = color || "white";
        viewDom.fill();
    },
    ...
    4: function(ps, viewDom, color) {
        ...
        var defX = psObj.x - (0, _config.pixelWeigth) / 2;
        viewDom.beginPath();
        viewDom.moveTo(defX + (0, _config.pixelWeigth), psObj.y);
        viewDom.lineTo(defX + (0, _config.pixelWeigth), psObj.y + (0, _config.pixelWeigth));
        viewDom.lineTo(defX, psObj.y + (0, _config.pixelWeigth) + 1);
        viewDom.fillStyle = color || "white";
        viewDom.fill();
    },
    5: function(ps, viewDom, color) {
        var psObj = (0, _positionMethod.positionToXY)(ps);
        var defX = psObj.x - (0, _config.pixelWeigth) / 2;
        var halfzWidth = (0, _config.pixelWeigth) / 2;
        viewDom.beginPath();
        viewDom.moveTo(defX, psObj.y);
        viewDom.lineTo(defX + (0, _config.pixelWeigth), psObj.y);
    ...
    b0: function(ps, viewDom, color) {
        ...
        viewDom.beginPath();
        viewDom.moveTo(defX, psObj.y);
        viewDom.lineTo(defX + halfzWidth, psObj.y + (0, _config.pixelWeigth));
        viewDom.lineTo(defX, psObj.y + (0, _config.pixelWeigth));
        viewDom.fillStyle = color || "white";
        viewDom.fill();
    },
    ...
    crystal4: function(ps, viewDom, color) {
        ...
        viewDom.moveTo(defX, psObj.y);
        viewDom.lineTo(defX + (0, _config.pixelWeigth), psObj.y);
        viewDom.lineTo(defX + (0, _config.pixelWeigth), psObj.y + halfzWidth);
        viewDom.lineTo(defX + halfzWidth, psObj.y + (0, _config.pixelWeigth));
        viewDom.lineTo(defX, psObj.y + halfzWidth);
        viewDom.fillStyle = color || "white";
        viewDom.fill();
...
exports.default = function(brickObj, viewDom, color) {
    if (!brickObj) return;
    if (typeof brickObj === "number") return brickMap[0](brickObj, viewDom, color);
    return brickMap[brickObj.brickType](brickObj.ps, viewDom, color);
};

},{"./positionMethod":"gqbqK","./config":"4Wc5b","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"kdkJ4":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "deadSound", ()=>deadSound);
parcelHelpers.export(exports, "explosionSound", ()=>explosionSound);
var _tone = require("tone");
var _toneDefault = parcelHelpers.interopDefault(_tone);
var synth = new (0, _toneDefault.default).FMSynth().toMaster();
function deadSound() {
    synth.triggerAttackRelease("C#2", .1, "+0");
    synth.triggerAttackRelease("C2", .05, "+.15");
}
var noise = new (0, _toneDefault.default).Noise({
    "volume": -Infinity,
    "type": "brown",
    "fadeIn": 500000,
    "fadeOut": 400
...
var autoFilter = new (0, _toneDefault.default).AutoFilter({
    "frequency": ".5",
    "min": 800,
    "max": 15000
}).toMaster();
...
},{"tone":"4oO3E","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"4oO3E":[function(require,module,exports) {
(function(root, factory) {
    ...
    Main(function() {
		 ...
		 *  @constructor
		 */ var Tone = function() {
            if (!(this instanceof Tone)) throw new Error("constructor needs to be called with the 'new' keyword");
        };
        /**
		 *  @memberOf Tone#
		 ...
		 */ Tone.prototype.toString = function() {
            for(var className in Tone){
                ...
                if (Tone.isFunction(Tone[className]) && isLetter && sameConstructor) return className;
            }
            return "Tone";
        };
        /**
		 ...
		 */ Tone.prototype.set = function(params, value, rampTime) {
            if (Tone.isObject(params)) rampTime = value;
            ...
            paramLoop: for(var attr in params){
                ...
                if (Tone.isUndef(param)) continue;
                if (Tone.Signal && param instanceof Tone.Signal || Tone.Param && param instanceof Tone.Param) {
                    if (param.value !== value) {
                        if (Tone.isUndef(rampTime)) param.value = value;
                        else param.rampTo(value, rampTime);
                    }
                } else if (param instanceof AudioParam) {
                    if (param.value !== value) param.value = value;
                } else if (Tone.TimeBase && param instanceof Tone.TimeBase) parent[attr] = value;
                else if (param instanceof Tone) param.set(value);
                else if (param !== value) parent[attr] = value;
            }
            return this;
        };
        /**
		 ...
		 */ Tone.prototype.get = function(params) {
            if (Tone.isUndef(params)) params = this._collectDefaults(this.constructor);
            else if (Tone.isString(params)) params = [
                params
            ];
            var ret = {};
            for(var i = 0; i < params.length; i++){
                ...
                var param = parent[attr];
                if (Tone.isObject(params[attr])) subRet[attr] = param.get();
                else if (Tone.Signal && param instanceof Tone.Signal) subRet[attr] = param.value;
                else if (Tone.Param && param instanceof Tone.Param) subRet[attr] = param.value;
                else if (param instanceof AudioParam) subRet[attr] = param.value;
                else if (param instanceof Tone) subRet[attr] = param.get();
                else if (!Tone.isFunction(param) && Tone.isDefined(param)) subRet[attr] = param;
            }
            return ret;
        };
        /**
		 ...
		 */ Tone.prototype._collectDefaults = function(constr) {
            var ret = [];
            if (Tone.isDefined(constr.defaults)) ret = Object.keys(constr.defaults);
            if (Tone.isDefined(constr._super)) {
                var superDefs = this._collectDefaults(constr._super);
                //filter out repeats
                for(var i = 0; i < superDefs.length; i++)if (ret.indexOf(superDefs[i]) === -1) ret.push(superDefs[i]);
            }
            return ret;
        };
        ///////////////////////////////////////////////////////////////////////////
		 ...
		 */ Tone.defaults = function(values, keys, constr) {
            var options = {};
            if (values.length === 1 && Tone.isObject(values[0])) options = values[0];
            else for(var i = 0; i < keys.length; i++)options[keys[i]] = values[i];
            if (Tone.isDefined(constr.defaults)) return Tone.defaultArg(options, constr.defaults);
            else if (Tone.isObject(constr)) return Tone.defaultArg(options, constr);
            else return options;
        };
        /**
		 ...
		 */ Tone.defaultArg = function(given, fallback) {
            if (Tone.isObject(given) && Tone.isObject(fallback)) {
                ...
                for(var givenProp in given)ret[givenProp] = Tone.defaultArg(fallback[givenProp], given[givenProp]);
                for(var fallbackProp in fallback)ret[fallbackProp] = Tone.defaultArg(given[fallbackProp], fallback[fallbackProp]);
                return ret;
            } else return Tone.isUndef(given) ? fallback : given;
        };
        ///////////////////////////////////////////////////////////////////////////
        //	CONNECTIONS
        ///////////////////////////////////////////////////////////////////////////
		 ...
		 */ Tone.prototype._readOnly = function(property) {
            if (Array.isArray(property)) for(var i = 0; i < property.length; i++)this._readOnly(property[i]);
            else Object.defineProperty(this, property, {
                writable: false,
                enumerable: true
            });
        };
		 ...
		 */ Tone.prototype._writable = function(property) {
            if (Array.isArray(property)) for(var i = 0; i < property.length; i++)this._writable(property[i]);
            else Object.defineProperty(this, property, {
                writable: true
            });
        };
		 ...
		 */ Tone.extend = function(child, parent) {
            if (Tone.isUndef(parent)) parent = Tone;
            function TempConstructor() {}
            TempConstructor.prototype = parent.prototype;
            child.prototype = new TempConstructor();
            /** @override */ child.prototype.constructor = child;
            child._super = parent;
        };
        ///////////////////////////////////////////////////////////////////////////
        //	CONTEXT
		 ...
		 */ Object.defineProperty(Tone, "context", {
            ...
            set: function(context) {
                if (Tone.Context && context instanceof Tone.Context) audioContext = context;
                else audioContext = new Tone.Context(context);
		 ...
		 */ Tone.getContext = function(resolve) {
            if (Tone.initialized) resolve(Tone.context);
            else {
		 ...
		 */ Tone.Emitter.prototype.off = function(event, callback) {
            ...
            for(var ev = 0; ev < events.length; ev++){
                ...
                if (this._events.hasOwnProperty(event)) {
                    if (Tone.isUndef(callback)) this._events[event] = [];
                    else {
                        var eventList = this._events[event];
                        for(var i = 0; i < eventList.length; i++)if (eventList[i] === callback) eventList.splice(i, 1);
                    }
                }
            }
            return this;
		 ...
		 */ Tone.Emitter.prototype.emit = function(event) {
            if (this._events) {
                ...
                if (this._events.hasOwnProperty(event)) {
                    ...
                    for(var i = 0, len = eventList.length; i < len; i++)eventList[i].apply(this, args);
                }
            }
            return this;
        };
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Timeline.prototype.add = function(event) {
            ...
            if (Tone.isUndef(event.time)) throw new Error("Tone.Timeline: events must have a time attribute");
		 ...
		 */ Tone.Timeline.prototype.getBefore = function(time, comparator) {
            ...
            if (len > 0 && this._timeline[len - 1][comparator] < time) return this._timeline[len - 1];
            var index = this._search(time, comparator);
		 ...
		 */ Tone.Timeline.prototype.cancel = function(after) {
                ...
                if (index >= 0) {
                    if (this._timeline[index].time === after) {
                        ...
                        for(var i = index; i >= 0; i--){
                            if (this._timeline[i].time === after) index = i;
                            else break;
                        }
            ...
            {
                if (this._timeline[0].time >= after) this._timeline = [];
            }
            return this;
        };
        /**
		 ...
		 */ Tone.Timeline.prototype._search = function(time, comparator) {
            ...
            var len = this._timeline.length;
            var end = len;
            if (len > 0 && this._timeline[len - 1][comparator] <= time) return len - 1;
            while(beginning < end){
                ...
                if (event[comparator] === time) {
                    ...
                    for(var i = midPoint; i < this._timeline.length; i++){
                        ...
                        if (testEvent[comparator] === time) midPoint = i;
                    }
                    return midPoint;
                } else if (event[comparator] < time && nextEvent[comparator] > time) return midPoint;
                else if (event[comparator] > time) //search lower
                end = midPoint;
                else //search upper
		 ...
		 */ Tone.Timeline.prototype.forEachAtTime = function(time, callback) {
            //iterate over the items in reverse so that removing an item doesn't break things
            var upperBound = this._search(time);
            if (upperBound !== -1) this._iterate(function(event) {
                if (event.time === time) callback.call(this, event);
            }, 0, upperBound);
            return this;
        };
        /**
    ...
    Module(function(Tone) {
        if (Tone.supported) {
            if (!window.hasOwnProperty("OfflineAudioContext") && window.hasOwnProperty("webkitOfflineAudioContext")) window.OfflineAudioContext = window.webkitOfflineAudioContext;
            //returns promise?
    ...
    Module(function(Tone) {
        if (Tone.supported) {
            if (!window.hasOwnProperty("AudioContext") && window.hasOwnProperty("webkitAudioContext")) window.AudioContext = window.webkitAudioContext;
            //not functionally equivalent, but only an API placeholder
            if (!AudioContext.prototype.close) AudioContext.prototype.close = function() {
                if (Tone.isFunction(this.suspend)) this.suspend();
                return Promise.resolve();
            };
            //not functionally equivalent
            if (!AudioContext.prototype.resume) AudioContext.prototype.resume = function() {
                return Promise.resolve();
            };
            //createGain
            if (!AudioContext.prototype.createGain && AudioContext.prototype.createGainNode) AudioContext.prototype.createGain = AudioContext.prototype.createGainNode;
            //createDelay
            if (!AudioContext.prototype.createDelay && AudioContext.prototype.createDelayNode) AudioContext.prototype.createDelay = AudioContext.prototype.createDelayNode;
            ...
            var audioData1 = new Uint32Array([
                1179011410,
                48,
                1163280727,
                544501094,
            ...
            try {
                var ret = offlineContext.decodeAudioData(audioData1);
                if (ret instanceof Promise) decodeAudioDataPromise = true;
            } catch (e) {
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Context = function() {
            ...
            if (!options.context) {
                ...
                if (!options.context) throw new Error("could not create AudioContext. Possibly too many AudioContexts running already.");
            }
		 ...
		 */ Tone.Context.prototype._defineProperty = function(context, prop) {
            if (Tone.isUndef(this[prop])) Object.defineProperty(this, prop, {
                get: function() {
                    if (typeof context[prop] === "function") return context[prop].bind(context);
                    else return context[prop];
                },
                set: function(val) {
                    context[prop] = val;
                }
		 ...
		 */ Tone.Context.prototype.ready = function() {
            return new Promise((function(done) {
                if (this._context.state === "running") done();
                else this._context.resume().then(function() {
                    done();
                });
            }).bind(this));
        };
		 ...
		 */ Tone.Context.prototype.getConstant = function(val) {
            if (this._constants[val]) return this._constants[val];
            else {
		 ...
		 */ Tone.Context.prototype._timeoutLoop = function() {
            var now = this.now();
            while(this._timeouts && this._timeouts.length && this._timeouts.peek().time <= now)this._timeouts.shift().callback();
        };
        /**
		 ...
		 */ Tone.Context.prototype.clearTimeout = function(id) {
            this._timeouts.forEach(function(event) {
                if (event.id === id) this.remove(event);
            });
            return this;
        };
        /**
		 ...
		 */ Object.defineProperty(Tone.Context.prototype, "latencyHint", {
            ...
            set: function(hint) {
                ...
                if (Tone.isString(hint)) switch(hint){
                    case "interactive":
                        lookAhead = 0.1;
		 ...
		 */ Tone.Context.prototype.dispose = function() {
            return this.close().then((function() {
                ...
                this._timeouts = null;
                for(var con in this._constants)this._constants[con].disconnect();
		 ...
		 */ Ticker.prototype._createWorker = function() {
            ...
            var blob = new Blob([
                "var timeoutTime = " + (this._updateInterval * 1000).toFixed(1) + ";" + "self.onmessage = function(msg){" + "	timeoutTime = parseInt(msg.data);" + "};" + //and schedules a new tick
                "function tick(){" + "	setTimeout(tick, timeoutTime);" + "	self.postMessage('tick');" + "}" + "tick();"
            ]);
		 ...
		 */ Ticker.prototype._createClock = function() {
            if (this._type === Ticker.Type.Worker) try {
                this._createWorker();
            } catch (e) {
            ...
            else if (this._type === Ticker.Type.Timeout) this._createTimeout();
        };
        /**
		 * @memberOf Ticker#
		 * @type {Number}
		 ...
		 */ Object.defineProperty(Ticker.prototype, "updateInterval", {
            ...
            set: function(interval) {
                ...
                if (this._type === Ticker.Type.Worker) this._worker.postMessage(Math.max(interval * 1000, 1));
            }
        });
        /**
		 ...
		 */ Tone.getContext(function() {
            ...
            function toneConnect(B, outNum, inNum) {
                if (B.input) {
                    ...
                    if (Tone.isArray(B.input)) return this.connect(B.input[inNum]);
                    else return this.connect(B.input, outNum, inNum);
                } else try {
            ...
            function toneDisconnect(B, outNum, inNum) {
                ...
                } else if (B && B.input) this.disconnect(B.input, outNum, inNum);
                else try {
        ...
        } else if (!Tone.supported) // eslint-disable-next-line no-console
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.AudioNode.prototype.createInsOuts = function(inputs, outputs) {
            if (inputs === 1) this.input = this.context.createGain();
            else if (inputs > 1) this.input = new Array(inputs);
            if (outputs === 1) this.output = this.context.createGain();
            else if (outputs > 1) this.output = new Array(outputs);
        };
        /**
		 ...
		 */ Object.defineProperty(Tone.AudioNode.prototype, "numberOfInputs", {
            get: function() {
                if (this.input) {
                    if (Tone.isArray(this.input)) return this.input.length;
                    else return 1;
                } else return 0;
            }
        });
		 ...
		 */ Object.defineProperty(Tone.AudioNode.prototype, "numberOfOutputs", {
            get: function() {
                if (this.output) {
                    if (Tone.isArray(this.output)) return this.output.length;
                    else return 1;
                } else return 0;
            }
        });
		 ...
		 */ Tone.AudioNode.prototype.connect = function(unit, outputNum, inputNum) {
            if (unit._onConnect) unit._onConnect(this);
            if (Tone.isArray(this.output)) {
                outputNum = Tone.defaultArg(outputNum, 0);
                this.output[outputNum].connect(unit, 0, inputNum);
            } else this.output.connect(unit, outputNum, inputNum);
            return this;
        };
        /**
		 ...
		 */ Tone.AudioNode.prototype.disconnect = function(destination, outputNum, inputNum) {
            if (Tone.isArray(this.output)) {
                if (Tone.isNumber(destination)) this.output[destination].disconnect();
                else {
                    outputNum = Tone.defaultArg(outputNum, 0);
                    this.output[outputNum].disconnect(destination, 0, inputNum);
                }
            } else this.output.disconnect.apply(this.output, arguments);
        };
        /**
		 ...
		 */ Tone.AudioNode.prototype.fan = function() {
            for(var i = 0; i < arguments.length; i++)this.connect(arguments[i]);
            return this;
        };
        if (window.AudioNode) {
		 ...
		 */ Tone.AudioNode.prototype.dispose = function() {
            if (Tone.isDefined(this.input)) {
                if (this.input instanceof AudioNode) this.input.disconnect();
                this.input = null;
            }
            if (Tone.isDefined(this.output)) {
                if (this.output instanceof AudioNode) this.output.disconnect();
                this.output = null;
            }
            this._context = null;
            return this;
        ...
        if (Tone.supported) {
            ...
            if (isSafari) {
                var WaveShaperNode = function(context) {
                    ...
                    this._curve = null;
                    for(var prop in this._internalNode)this._defineProperty(this._internalNode, prop);
                };
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.WaveShaper = function(mapping, bufferLen) {
			 ...
			 *  @private
			 */ this._curve = null;
            if (Array.isArray(mapping)) this.curve = mapping;
            else if (isFinite(mapping) || Tone.isUndef(mapping)) this._curve = new Float32Array(Tone.defaultArg(mapping, 1024));
		 ...
		 */ Object.defineProperty(Tone.WaveShaper.prototype, "oversample", {
            ...
            set: function(oversampling) {
                if ([
                    ...
                    "2x",
                    "4x"
                ].includes(oversampling)) this._shaper.oversample = oversampling;
                else throw new RangeError("Tone.WaveShaper: oversampling must be either 'none', '2x', or '4x'");
            }
        });
        /**
		 *  Clean up.
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.TimeBase = function(val, units) {
            ...
            if (this instanceof Tone.TimeBase) {
                ...
                if (Tone.isUndef(this._units) && Tone.isString(this._val) && parseFloat(this._val) == this._val && this._val.charAt(0) !== "+") {
                ...
                } else if (val instanceof Tone.TimeBase) switch(this._defaultUnits){
                    case "s":
            ...
            } else return new Tone.TimeBase(val, units);
        };
		 ...
		 */ Tone.TimeBase.prototype._expressions = {
            "n": {
                ...
                method: function(value, dot) {
                    ...
                    if (value === 1) return this._beatsToUnits(this._getTimeSignature()) * scalar;
                    else return this._beatsToUnits(4 / value) * scalar;
                }
            },
            "t": {
                regexp: /^(\d+)t$/i,
		 ...
		 */ Tone.TimeBase.prototype._getBpm = function() {
            if (Tone.Transport) return Tone.Transport.bpm.value;
            else return 120;
        };
        /**
		 ...
		 */ Tone.TimeBase.prototype._getTimeSignature = function() {
            if (Tone.Transport) return Tone.Transport.timeSignature;
            else return 4;
        };
        /**
		 ...
		 */ Tone.TimeBase.prototype._getPPQ = function() {
            if (Tone.Transport) return Tone.Transport.PPQ;
            else return 192;
        };
        /**
		 ...
		 */ Tone.TimeBase.prototype.valueOf = function() {
            if (Tone.isUndef(this._val)) return this._noArg();
            else if (Tone.isString(this._val) && Tone.isUndef(this._units)) {
                for(var units in this._expressions)if (this._expressions[units].regexp.test(this._val.trim())) {
                    this._units = units;
                    break;
                }
            }
            if (Tone.isDefined(this._units)) {
                ...
                else return expr.method.call(this, parseFloat(this._val));
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Frequency = function(val, units) {
            if (this instanceof Tone.Frequency) Tone.TimeBase.call(this, val, units);
            else return new Tone.Frequency(val, units);
        };
        ...
        Tone.Frequency.prototype._expressions = Object.assign({}, Tone.TimeBase.prototype._expressions, {
            "midi": {
                regexp: /^(\d+(?:\.\d+)?midi)/,
                method: function(value) {
                    if (this._defaultUnits === "midi") return value;
                    else return Tone.Frequency.mtof(value);
                }
            },
            "note": {
                ...
                method: function(pitch, octave) {
                    ...
                    if (this._defaultUnits === "midi") return noteNumber;
                    else return Tone.Frequency.mtof(noteNumber);
                }
            },
            "tr": {
                regexp: /^(\d+(?:\.\d+)?):(\d+(?:\.\d+)?):?(\d+(?:\.\d+)?)?/,
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Time = function(val, units) {
            if (this instanceof Tone.Time) Tone.TimeBase.call(this, val, units);
            else return new Tone.Time(val, units);
        };
		 ...
		 */ Tone.Time.prototype.toBarsBeatsSixteenths = function() {
            ...
            if (sixteenths.length > 3) // the additional parseFloat removes insignificant trailing zeroes
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.TransportTime = function(val, units) {
            if (this instanceof Tone.TransportTime) Tone.Time.call(this, val, units);
            else return new Tone.TransportTime(val, units);
        };
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.prototype.toSeconds = function(time) {
            if (Tone.isNumber(time)) return time;
            else if (Tone.isUndef(time)) return this.now();
            else if (Tone.isString(time)) return new Tone.Time(time).toSeconds();
            else if (time instanceof Tone.TimeBase) return time.toSeconds();
        };
        /**
		 ...
		 */ Tone.prototype.toFrequency = function(freq) {
            if (Tone.isNumber(freq)) return freq;
            else if (Tone.isString(freq) || Tone.isUndef(freq)) return new Tone.Frequency(freq).valueOf();
            else if (freq instanceof Tone.TimeBase) return freq.toFrequency();
        };
        /**
		 ...
		 */ Tone.prototype.toTicks = function(time) {
            if (Tone.isNumber(time) || Tone.isString(time)) return new Tone.TransportTime(time).toTicks();
            else if (Tone.isUndef(time)) return Tone.Transport.ticks;
            else if (time instanceof Tone.TimeBase) return time.toTicks();
        };
        return Tone;
    });
    Module(function(Tone) {
		 ...
		 */ Tone.Param = function() {
            ...
            if (Tone.isDefined(options.value) && this._param) this.value = options.value;
        };
		 ...
		 */ Object.defineProperty(Tone.Param.prototype, "minValue", {
            get: function() {
                if (this.units === Tone.Type.Time || this.units === Tone.Type.Frequency || this.units === Tone.Type.NormalRange || this.units === Tone.Type.Positive || this.units === Tone.Type.BPM) return 0;
                else if (this.units === Tone.Type.AudioRange) return -1;
                else if (this.units === Tone.Type.Decibels) return -Infinity;
		 ...
		 */ Object.defineProperty(Tone.Param.prototype, "maxValue", {
            get: function() {
                if (this.units === Tone.Type.NormalRange || this.units === Tone.Type.AudioRange) return 1;
		 ...
		 */ Tone.Param.prototype._fromUnits = function(val) {
            if ((this.convert || Tone.isUndef(this.convert)) && !this.overridden) switch(this.units){
                case Tone.Type.Time:
		 ...
		 */ Tone.Param.prototype._toUnits = function(val) {
            if (this.convert || Tone.isUndef(this.convert)) switch(this.units){
                case Tone.Type.Decibels:
		 ...
		 */ Tone.Param.prototype.getValueAtTime = function(time) {
            ...
            //if it was set by
            if (before === null) value = initialValue;
            else if (before.type === Tone.Param.AutomationType.Target) {
                var previous = this._events.getBefore(before.time);
                var previousVal;
                if (previous === null) previousVal = initialValue;
            ...
            else if (after.type === Tone.Param.AutomationType.Linear) value = this._linearInterpolate(before.time, before.value, after.time, after.value, time);
            else if (after.type === Tone.Param.AutomationType.Exponential) value = this._exponentialInterpolate(before.time, before.value, after.time, after.value, time);
		 ...
		 */ Tone.Param.prototype.setRampPoint = function(time) {
            ...
            if (currentVal === 0) currentVal = this._minOutput;
		 ...
		 */ Tone.Param.prototype.setTargetAtTime = function(value, startTime, timeConstant) {
            ...
            if (timeConstant <= 0) throw new Error("timeConstant must be greater than 0");
            startTime = this.toSeconds(startTime);
            this._events.add({
		 ...
		 */ Tone.Param.prototype.setValueCurveAtTime = function(values, startTime, duration, scaling) {
            ...
            for(var i = 1; i < values.length; i++)this.linearRampToValueAtTime(values[i] * scaling, startTime + i * segTime);
            return this;
        };
        /**
		 ...
		 */ Tone.Param.prototype.cancelAndHoldAtTime = function(time) {
            ...
            if (before && before.time === time) {
                ...
                if (after) this._events.cancel(after.time);
                else this._events.cancel(time + 0.000001);
            } else if (after) {
                //cancel the next event(s)
                this._events.cancel(after.time);
                if (!this._param.cancelAndHoldAtTime) this._param.cancelScheduledValues(time);
                if (after.type === Tone.Param.AutomationType.Linear) {
                    if (!this._param.cancelAndHoldAtTime) this.linearRampToValueAtTime(valueAtTime, time);
                    else this._events.add({
                        "type": Tone.Param.AutomationType.Linear,
                        "value": valueAtTime,
                        "time": time
                    });
                } else if (after.type === Tone.Param.AutomationType.Exponential) {
                    if (!this._param.cancelAndHoldAtTime) this.exponentialRampToValueAtTime(valueAtTime, time);
                    else this._events.add({
                        "type": Tone.Param.AutomationType.Exponential,
                        "value": valueAtTime,
                        "time": time
            ...
            if (this._param.cancelAndHoldAtTime) this._param.cancelAndHoldAtTime(time);
            else this._param.setValueAtTime(valueAtTime, time);
            return this;
        };
        /**
		 ...
		 */ Tone.Param.prototype.rampTo = function(value, rampTime, startTime) {
            ...
            if (this.units === Tone.Type.Frequency || this.units === Tone.Type.BPM || this.units === Tone.Type.Decibels) this.exponentialRampTo(value, rampTime, startTime);
    ...
    Module(function(Tone) {
        if (Tone.supported) {
            ...
            if (isMobileSafari) //mobile safari has a bizarre bug with the offline context
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Signal.prototype._connectProxies = function() {
            ...
            this._proxies.forEach((function(proxy) {
                ...
                if (proxy._proxies) proxy._connectProxies();
            }).bind(this));
        };
        /**
		 ...
		 */ Tone.Signal.prototype._onConnect = function(from) {
            if (!this._isParam(from)) //connect all the proxies
		 ...
		 */ Tone.Signal.prototype.disconnect = function(node) {
            ...
            } else if (!node) //no argument, disconnect everything
            this._proxies = [];
		 ...
		 */ Tone.Signal.prototype.getValueAtTime = function(time) {
            if (this._param.getValueAtTime) return this._param.getValueAtTime(time);
            else return Tone.Param.prototype.getValueAtTime.call(this, time);
        };
    ...
    Module(function(Tone) {
		 ...
		 */ Object.defineProperty(Tone.Envelope.prototype, "attackCurve", {
            get: function() {
                if (Tone.isString(this._attackCurve)) return this._attackCurve;
                else if (Tone.isArray(this._attackCurve)) {
                    //look up the name in the curves array
                    for(var type in Tone.Envelope.Type){
                        if (Tone.Envelope.Type[type].In === this._attackCurve) return type;
                    }
            ...
            set: function(curve) {
                ...
                if (Tone.Envelope.Type.hasOwnProperty(curve)) {
                    ...
                    if (Tone.isObject(curveDef)) this._attackCurve = curveDef.In;
                    else this._attackCurve = curveDef;
                } else if (Tone.isArray(curve)) this._attackCurve = curve;
                else throw new Error("Tone.Envelope: invalid curve: " + curve);
            }
        });
        /**
		 ...
		 */ Object.defineProperty(Tone.Envelope.prototype, "releaseCurve", {
            get: function() {
                if (Tone.isString(this._releaseCurve)) return this._releaseCurve;
                else if (Tone.isArray(this._releaseCurve)) {
                    //look up the name in the curves array
                    for(var type in Tone.Envelope.Type){
                        if (Tone.Envelope.Type[type].Out === this._releaseCurve) return type;
                    }
                    //otherwise just return the array
            ...
            set: function(curve) {
                ...
                if (Tone.Envelope.Type.hasOwnProperty(curve)) {
                    var curveDef = Tone.Envelope.Type[curve];
                    if (Tone.isObject(curveDef)) this._releaseCurve = curveDef.Out;
                    else this._releaseCurve = curveDef;
                } else if (Tone.isArray(curve)) this._releaseCurve = curve;
                else throw new Error("Tone.Envelope: invalid curve: " + curve);
            }
        });
        /**
		 ...
		 */ Tone.Envelope.prototype.triggerAttack = function(time, velocity) {
            ...
            //attack
            if (this._attackCurve === "linear") this._sig.linearRampTo(velocity, attack, time);
            ...
            //decay
            if (decay) this._sig.targetRampTo(velocity * this.sustain, decay, attack + time);
            return this;
        };
        /**
		 ...
		 */ Tone.Envelope.prototype.triggerRelease = function(time) {
            ...
            if (currentValue > 0) {
                ...
                if (this._releaseCurve === "linear") this._sig.linearRampTo(0, release, time);
        ...
        /**
	 	 ...
	 	 */ (function _createCurves() {
			 ...
			 */ function invertCurve(curve) {
                ...
                for(var j = 0; j < curve.length; j++)out[j] = 1 - curve[j];
                return out;
            }
            /**
			 *  reverse the curve
    ...
    Module(function(Tone) {
		 ...
		 *  @private
		 */ if (Tone.supported) {
            if (!AnalyserNode.prototype.getFloatTimeDomainData) //referenced https://github.com/mohayonao/get-float-time-domain-data
            AnalyserNode.prototype.getFloatTimeDomainData = function(array) {
                var uint8 = new Uint8Array(array.length);
                this.getByteTimeDomainData(uint8);
                for(var i = 0; i < uint8.length; i++)array[i] = (uint8[i] - 128) / 128;
            };
        }
    });
    Module(function(Tone) {
		 ...
		 */ Tone.Analyser.prototype.getValue = function() {
            if (this._type === Tone.Analyser.Type.FFT) this._analyser.getFloatFrequencyData(this._buffer);
            else if (this._type === Tone.Analyser.Type.Waveform) this._analyser.getFloatTimeDomainData(this._buffer);
            return this._buffer;
        };
        /**
		 ...
		 */ Object.defineProperty(Tone.Analyser.prototype, "type", {
            ...
            set: function(type) {
                if (type !== Tone.Analyser.Type.Waveform && type !== Tone.Analyser.Type.FFT) throw new TypeError("Tone.Analyser: invalid type: " + type);
                this._type = type;
            }
        });
        /**
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.EqualPowerGain = function() {
			 ...
			 */ this._eqPower = this.input = this.output = new Tone.WaveShaper((function(val) {
                if (Math.abs(val) < 0.001) //should output 0 when input is 0
                return 0;
                else return Tone.equalPowerScale(val);
            }).bind(this), 4096);
        };
    ...
    Module(function(Tone) {
		 ...
		 */ Object.defineProperty(Tone.Filter.prototype, "type", {
            ...
            set: function(type) {
                ...
                if (types.indexOf(type) === -1) throw new TypeError("Tone.Filter: invalid type " + type);
                this._type = type;
                for(var i = 0; i < this._filters.length; i++)this._filters[i].type = type;
            }
        });
        /**
		 ...
		 */ Object.defineProperty(Tone.Filter.prototype, "rolloff", {
            ...
            set: function(rolloff) {
                ...
                if (cascadingCount === -1) throw new RangeError("Tone.Filter: rolloff can only be -12, -24, -48 or -96");
                cascadingCount += 1;
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.TickSignal.prototype._getTicksUntilEvent = function(event, time) {
            ...
            if (this._events.get(time).time === time && this._events.get(time).type === Tone.Param.AutomationType.SetValue) val1 = this.getValueAtTime(time - this.sampleTime);
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.TimelineState.prototype.getNextState = function(state, time) {
            time = this.toSeconds(time);
            var index = this._search(time);
            if (index !== -1) for(var i = index; i < this._timeline.length; i++){
                var event = this._timeline[i];
                if (event.state === state) return event;
            }
        };
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.TickSource.prototype.start = function(time, offset) {
            ...
            if (this._state.getValueAtTime(time) !== Tone.State.Started) {
                ...
                if (Tone.isDefined(offset)) this.setTicksAtTime(offset, time);
            }
            return this;
        };
        /**
		 ...
		 */ Tone.TickSource.prototype.pause = function(time) {
            time = this.toSeconds(time);
            if (this._state.getValueAtTime(time) === Tone.State.Started) this._state.setStateAtTime(Tone.State.Paused, time);
            return this;
        };
        /**
		 ...
		 */ Tone.TickSource.prototype.getTicksAtTime = function(time) {
            ...
            this._state.forEachBetween(stopEvent.time, time + this.sampleTime, (function(e) {
                ...
                if (lastState.state === Tone.State.Started && e.state !== Tone.State.Started) elapsedTicks += this.frequency.getTicksAtTime(e.time) - this.frequency.getTicksAtTime(periodStartTime);
                lastState = e;
            }).bind(this));
		 ...
		 */ Tone.TickSource.prototype.getSecondsAtTime = function(time) {
            ...
            this._state.forEachBetween(stopEvent.time, time + this.sampleTime, (function(e) {
                ...
                if (lastState.state === Tone.State.Started && e.state !== Tone.State.Started) elapsedSeconds += e.time - periodStartTime;
                lastState = e;
            }).bind(this));
            //remove the temporary event
            this._state.remove(tmpEvent);
		 ...
		 */ Tone.TickSource.prototype.forEachTickBetween = function(startTime, endTime, callback) {
            ...
            this._state.forEachBetween(startTime, endTime, (function(event) {
                if (lastStateEvent.state === Tone.State.Started && event.state !== Tone.State.Started) this.forEachTickBetween(Math.max(lastStateEvent.time, startTime), event.time - this.sampleTime, callback);
            ...
            if (lastStateEvent.state === Tone.State.Started && this._state) {
                ...
                while(nextTickTime < endTime && this._state){
                    ...
                    if (this._state) nextTickTime += this.frequency.getDurationOfTicks(1, nextTickTime);
                }
            }
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Clock.prototype.start = function(time, offset) {
            ...
            if (this._state.getValueAtTime(time) !== Tone.State.Started) {
                ...
                if (time < this._lastUpdate) this.emit("start", time, offset);
            }
            return this;
        };
        /**
		 ...
		 */ Tone.Clock.prototype.stop = function(time) {
            ...
            if (time < this._lastUpdate) this.emit("stop", time);
            return this;
        };
        /**
		 *  Pause the clock. Pausing does not reset the tick counter.
		 ...
		 */ Tone.Clock.prototype.pause = function(time) {
            ...
            if (this._state.getValueAtTime(time) === Tone.State.Started) {
                ...
                if (time < this._lastUpdate) this.emit("pause", time);
            }
            return this;
        };
        /**
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.IntervalTimeline.prototype.add = function(event) {
            if (Tone.isUndef(event.time) || Tone.isUndef(event.duration)) throw new Error("Tone.IntervalTimeline: events must have time and duration parameters");
            event.time = event.time.valueOf();
		 ...
		 */ Tone.IntervalTimeline.prototype.get = function(time) {
            if (this._root !== null) {
                ...
                if (results.length > 0) {
                    var max = results[0];
                    for(var i = 1; i < results.length; i++)if (results[i].low > max.low) max = results[i];
                    return max.event;
                }
            }
            return null;
		 ...
		 */ Tone.IntervalTimeline.prototype.dispose = function() {
            var allNodes = [];
            if (this._root !== null) this._root.traverse(function(node) {
                allNodes.push(node);
            });
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Ticks = function(val, units) {
            if (this instanceof Tone.Ticks) Tone.TransportTime.call(this, val, units);
            else return new Tone.Ticks(val, units);
        };
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.TransportEvent.prototype.invoke = function(time) {
            if (this.callback) {
                this.callback(time);
                if (this._once && this.Transport) this.Transport.clear(this.id);
            }
        };
        /**
		 * Clean up
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Transport.prototype.start = function(time, offset) {
            //start the clock
            if (Tone.isDefined(offset)) offset = this.toTicks(offset);
		 ...
		 */ Tone.Transport.prototype.toggle = function(time) {
            time = this.toSeconds(time);
            if (this._clock.getStateAtTime(time) !== Tone.State.Started) this.start(time);
            else this.stop(time);
            return this;
        };
        ///////////////////////////////////////////////////////////////////////////////
		 ...
		 */ Object.defineProperty(Tone.Transport.prototype, "timeSignature", {
            ...
            set: function(timeSig) {
                if (Tone.isArray(timeSig)) timeSig = timeSig[0] / timeSig[1] * 4;
		 ...
		 */ Tone.Transport.prototype.nextSubdivision = function(subdivision) {
            ...
            if (this.state !== Tone.State.Started) //if the transport's not started, return 0
            return 0;
            else {
                var now = this.now();
		 ...
		 */ Tone.Transport.prototype.syncSignal = function(signal, ratio) {
            if (!ratio) {
                //get the sync ratio
                var now = this.now();
                if (signal.getValueAtTime(now) !== 0) ratio = signal.getValueAtTime(now) / this.bpm.getValueAtTime(now);
                else ratio = 0;
            }
        ...
        Tone.Context.on("init", function(context) {
            if (context.Transport instanceof TransportConstructor) Tone.Transport = context.Transport;
            else Tone.Transport = new TransportConstructor();
            //store the Transport on the context so it can be retrieved later
            context.Transport = Tone.Transport;
        });
        Tone.Context.on("close", function(context) {
            if (context.Transport instanceof TransportConstructor) context.Transport.dispose();
        });
    ...
    Module(function(Tone) {
        ...
        if (window.AudioNode) // Also augment AudioNode's prototype to include toMaster as a convenience
        ...
        Tone.Context.on("init", function(context) {
            ...
            if (context.Master instanceof MasterConstructor) Tone.Master = context.Master;
            else Tone.Master = new MasterConstructor();
            context.Master = Tone.Master;
        });
        Tone.Context.on("close", function(context) {
            if (context.Master instanceof MasterConstructor) context.Master.dispose();
        });
        return Tone.Master;
    });
    Module(function(Tone) {
		 ...
		 */ Object.defineProperty(Tone.Source.prototype, "state", {
            get: function() {
                if (this._synced) {
                    if (Tone.Transport.state === Tone.State.Started) return this._state.getValueAtTime(Tone.Transport.seconds);
                    else return Tone.State.Stopped;
		 ...
		 */ Tone.Source.prototype.start = function(time, offset, duration) {
            if (Tone.isUndef(time) && this._synced) time = Tone.Transport.seconds;
            else time = this.toSeconds(time);
            ...
            if (this._state.getValueAtTime(time) === Tone.State.Started) {
                ...
                if (this._synced) {
                    ...
                    if (Tone.Transport.state === Tone.State.Started) this._syncedStart(this.now(), Tone.Transport.seconds);
		 ...
		 */ Tone.Source.prototype.stop = function(time) {
            ...
            if (!this._synced) this._stop.apply(this, arguments);
            else {
		 ...
		 */ Tone.Source.prototype.sync = function() {
            ...
            this._syncedStop = (function(time) {
                ...
                if (this._state.getValueAtTime(seconds) === Tone.State.Started) this._stop(time);
            }).bind(this);
		 ...
		 */ if (Tone.supported) {
            if (!AudioBuffer.prototype.copyToChannel) {
                AudioBuffer.prototype.copyToChannel = function(src, chanNum, start) {
                    ...
                    start = start || 0;
                    for(var i = 0; i < channel.length; i++)channel[i + start] = src[i];
                };
                AudioBuffer.prototype.copyFromChannel = function(dest, chanNum, start) {
                    var channel = this.getChannelData(chanNum);
                    start = start || 0;
                    for(var i = 0; i < dest.length; i++)dest[i] = channel[i + start];
                };
            }
        }
    });
    Module(function(Tone) {
		 ...
		 */ Tone.Buffer = function() {
            ...
            if (options.url instanceof AudioBuffer || options.url instanceof Tone.Buffer) {
                ...
                if (options.onload) {
                    if (this.loaded) options.onload(this);
                    else this._onload = options.onload;
                }
            } else if (Tone.isString(options.url)) this.load(options.url).then(options.onload).catch(options.onerror);
        };
		 ...
		 */ Tone.Buffer.prototype.load = function(url, onload, onerror) {
            var promise = new Promise((function(load, error) {
                this._xhr = Tone.Buffer.load(url, (function(buff) {
                    this._xhr = null;
                    this.set(buff);
                    load(this);
                    this._onload(this);
		 ...
		 */ Tone.Buffer.prototype.fromArray = function(array) {
            ...
            if (!isMultidimensional && channels === 1) array = [
                array
            ];
            for(var c = 0; c < channels; c++)buffer.copyToChannel(array[c], c);
            this._buffer = buffer;
            return this;
        };
        /**
		 ...
		 */ Tone.Buffer.prototype.toMono = function(chanNum) {
            if (Tone.isNumber(chanNum)) this.fromArray(this.toArray(chanNum));
            else {
                var outputArray = new Float32Array(this.length);
                var numChannels = this.numberOfChannels;
                for(var channel = 0; channel < numChannels; channel++){
                    var channelArray = this.toArray(channel);
                    for(var i = 0; i < channelArray.length; i++)outputArray[i] += channelArray[i];
                }
		 ...
		 */ Tone.Buffer.prototype.toArray = function(channel) {
            if (Tone.isNumber(channel)) return this.getChannelData(channel);
            else if (this.numberOfChannels === 1) return this.toArray(0);
            else {
                var ret = [];
                for(var c = 0; c < this.numberOfChannels; c++)ret[c] = this.getChannelData(c);
                return ret;
            }
        };
        /**
		 ...
		 */ Tone.Buffer.prototype.slice = function(start, end) {
            ...
            var replacement = [];
            for(var i = 0; i < this.numberOfChannels; i++)replacement[i] = this.toArray(i).slice(startSamples, endSamples);
		 ...
		 */ Tone.Buffer.prototype._reverse = function() {
            if (this.loaded) for(var i = 0; i < this.numberOfChannels; i++)Array.prototype.reverse.call(this.getChannelData(i));
            return this;
        };
        /**
		 * Reverse the buffer.
		 ...
		 */ Tone.Buffer._removeFromDownloadQueue = function(request) {
            ...
            if (index !== -1) Tone.Buffer._downloadQueue.splice(index, 1);
        };
        /**
		 ...
		 */ Tone.Buffer.load = function(url, onload, onerror) {
            ...
            if (matches) {
                ...
                for(var i = 0; i < extensions.length; i++)if (Tone.Buffer.supportsType(extensions[i])) {
            ...
            function onProgress() {
                ...
                var totalProgress = 0;
                for(var i = 0; i < Tone.Buffer._downloadQueue.length; i++)totalProgress += Tone.Buffer._downloadQueue[i].progress;
            ...
            request.addEventListener("load", function() {
                if (request.status === 200) Tone.context.decodeAudioData(request.response).then(function(buff) {
                    request.progress = 1;
                    onProgress();
                    onload(buff);
                    Tone.Buffer._removeFromDownloadQueue(request);
                    if (Tone.Buffer._downloadQueue.length === 0) //emit the event at the end
                    Tone.Buffer.emit("load");
                }).catch(function() {
                    Tone.Buffer._removeFromDownloadQueue(request);
                    onError("Tone.Buffer: could not decode audio data: " + url);
                });
                else onError("Tone.Buffer: could not locate file: " + url);
            });
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.OscillatorNode.prototype.getStateAtTime = function(time) {
            time = this.toSeconds(time);
            if (this._startTime !== -1 && time >= this._startTime && (this._stopTime === -1 || time <= this._stopTime)) return Tone.State.Started;
            else return Tone.State.Stopped;
        };
        /**
	     ...
	     */ Tone.OscillatorNode.prototype.start = function(time) {
            ...
            } else throw new Error("cannot call OscillatorNode.start more than once");
            return this;
        };
        /**
    ...
    Module(function(Tone) {
		 ...
		 */ Object.defineProperty(Tone.Oscillator.prototype, "type", {
            ...
            set: function(type) {
                ...
                if (this._phase === 0 && isBasicType) {
                    ...
                    if (this._oscillator !== null) this._oscillator.type;
                } else {
		 ...
		 */ Tone.Oscillator.prototype._inverseFFT = function(real, imag, phase) {
            var sum = 0;
            var len = real.length;
            for(var i = 0; i < len; i++)sum += real[i] * Math.cos(i * phase) + imag[i] * Math.sin(i * phase);
            return sum;
        };
        /**
		 ...
		 */ Tone.Oscillator.prototype._getInitialValue = function() {
            ...
            for(var i = 0; i < 8; i++)maxValue = Math.max(this._inverseFFT(real, imag, i / 8 * twoPi), maxValue);
		 ...
		 */ Object.defineProperty(Tone.Oscillator.prototype, "partials", {
            get: function() {
                if (this._type !== Tone.Oscillator.Type.Custom) return [];
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Panner3D.prototype.setPosition = function(x, y, z) {
            ...
            } else this._panner.setPosition(x, y, z);
		 ...
		 */ Tone.Panner3D.prototype.setOrientation = function(x, y, z) {
            ...
            } else this._panner.setOrientation(x, y, z);
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Solo.prototype._addSolo = function() {
            if (!Tone.isArray(this.context._currentSolo)) this.context._currentSolo = [];
            if (!this._isSoloed()) this.context._currentSolo.push(this);
        };
        /**
		 ...
		 */ Tone.Solo.prototype._isSoloed = function() {
            if (Tone.isArray(this.context._currentSolo)) return this.context._currentSolo.length !== 0 && this.context._currentSolo.indexOf(this) !== -1;
            else return false;
        };
        /**
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.CtrlInterpolate.prototype._interpolate = function(index, lower, upper) {
            if (Tone.isArray(lower)) {
                var retArray = [];
                for(var i = 0; i < lower.length; i++)retArray[i] = this._interpolate(index, lower[i], upper[i]);
                return retArray;
            } else if (Tone.isObject(lower)) {
                var retObj = {};
                for(var attr in lower)retObj[attr] = this._interpolate(index, lower[attr], upper[attr]);
                return retObj;
            } else {
		 ...
		 */ Tone.CtrlInterpolate.prototype._toNumber = function(val) {
            if (Tone.isNumber(val)) return val;
            else //otherwise assume that it's Time...
            return this.toSeconds(val);
        };
        /**
		 *  Clean up
    ...
    Module(function(Tone) {
                ...
                if (Tone.isArray(next)) {
                    ...
                    for(var i = 0; i < distribution.length; i++){
                        ...
                        if (rand > total && rand < total + dist) {
                            var chosen = next[i];
                            if (Tone.isObject(chosen)) this.value = chosen.value;
                            else this.value = chosen;
                        }
                        total += dist;
                    }
		 ...
		 */ Tone.CtrlMarkov.prototype._getProbDistribution = function(options) {
            ...
            for(var i = 0; i < options.length; i++){
                ...
                } else distribution[i] = 1 / options.length;
                total += distribution[i];
            }
            if (needsNormalizing) //normalize the values
            for(var j = 0; j < distribution.length; j++)distribution[j] = distribution[j] / total;
            return distribution;
        };
        /**
		 *  Clean up
    ...
    Module(function(Tone) {
		 ...
		 */ Object.defineProperty(Tone.CtrlPattern.prototype, "value", {
            get: function() {
                ...
                else if (this.values.length === 1) return this.values[0];
                this.index = Math.min(this.index, this.values.length - 1);
                var val = this.values[this.index];
                if (this.type === Tone.CtrlPattern.Type.RandomOnce) {
                    if (this.values.length !== this._shuffled.length) this._shuffleValues();
		 ...
		 */ Object.defineProperty(Tone.CtrlPattern.prototype, "type", {
            ...
            set: function(type) {
                ...
                this._shuffled = null;
                //the first index
                if (this._type === Tone.CtrlPattern.Type.Up || this._type === Tone.CtrlPattern.Type.UpDown || this._type === Tone.CtrlPattern.Type.RandomOnce || this._type === Tone.CtrlPattern.Type.AlternateUp) this.index = 0;
                else if (this._type === Tone.CtrlPattern.Type.Down || this._type === Tone.CtrlPattern.Type.DownUp || this._type === Tone.CtrlPattern.Type.AlternateDown) this.index = this.values.length - 1;
                //the direction
                if (this._type === Tone.CtrlPattern.Type.UpDown || this._type === Tone.CtrlPattern.Type.AlternateUp) this._direction = Tone.CtrlPattern.Type.Up;
                else if (this._type === Tone.CtrlPattern.Type.DownUp || this._type === Tone.CtrlPattern.Type.AlternateDown) this._direction = Tone.CtrlPattern.Type.Down;
                //randoms
                if (this._type === Tone.CtrlPattern.Type.RandomOnce) this._shuffleValues();
                else if (this._type === Tone.CtrlPattern.Random) this.index = Math.floor(Math.random() * this.values.length);
            }
        });
        /**
		 ...
		 */ Tone.CtrlPattern.prototype.next = function() {
            ...
            if (type === Tone.CtrlPattern.Type.Up) {
                ...
                if (this._direction === Tone.CtrlPattern.Type.Up) this.index++;
                else this.index--;
                if (this.index < 0) {
                    this.index = 1;
            ...
            } else if (type === Tone.CtrlPattern.Type.Random) this.index = Math.floor(Math.random() * this.values.length);
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Buffers.prototype.get = function(name) {
            if (this.has(name)) return this._buffers[name];
            else throw new Error("Tone.Buffers: no buffer named " + name);
        };
        /**
		 ...
		 */ Tone.Buffers.prototype._bufferLoaded = function(callback) {
            this._loadingCount--;
            if (this._loadingCount === 0 && callback) callback(this);
        };
        /**
		 ...
		 */ Tone.Buffers.prototype.add = function(name, url, callback) {
            ...
            if (url instanceof Tone.Buffer) {
                ...
                callback(this);
            } else if (Tone.isString(url)) this._buffers[name] = new Tone.Buffer(this.baseUrl + url, callback);
            return this;
        };
        /**
		 *  Clean up.
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.prototype.send = function(channelName, amount) {
            if (!Buses.hasOwnProperty(channelName)) Buses[channelName] = this.context.createGain();
        ...
        Tone.Context.on("init", function(context) {
            if (context.Buses) Buses = context.Buses;
            else {
                Buses = {};
                context.Buses = Buses;
            }
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Draw.prototype.schedule = function(callback, time) {
            ...
            if (this._events.length === 1) requestAnimationFrame(this._boundDrawLoop);
            return this;
        };
        /**
		 ...
		 */ Tone.Draw.prototype._drawLoop = function() {
            ...
            if (this._events.length > 0) requestAnimationFrame(this._boundDrawLoop);
        };
        //make a singleton
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Listener.prototype.setPosition = function(x, y, z) {
            ...
            } else this.context.listener.setPosition(x, y, z);
            this._position = Array.prototype.slice.call(arguments);
            return this;
        };
        /**
		 ...
		 */ Tone.Listener.prototype.setOrientation = function(x, y, z, upX, upY, upZ) {
            ...
            } else this.context.listener.setOrientation(x, y, z, upX, upY, upZ);
            this._orientation = Array.prototype.slice.call(arguments);
            return this;
        };
        /**
        ...
        Tone.Context.on("init", function(context) {
            if (context.Listener instanceof ListenerConstructor) //a single listener object
            Tone.Listener = context.Listener;
            else //make new Listener insides
            Tone.Listener = new ListenerConstructor();
    ...
    Module(function(Tone) {
		 ...
		 */ function attemptRender(callback, duration, sampleRate, tries) {
            ...
            if (context.currentTime > 0 && tries < 1000) return attemptRender(callback, duration, sampleRate, ++tries);
            else return {
                "response": response,
                "context": context
            };
		 ...
		 */ Tone.Offline = function(callback, duration) {
            ...
            var ret;
            if (response instanceof Promise) //wait for the promise to resolve
            ret = response.then(function() {
                //then render the audio
                return context.render();
            });
            else //process the audio
            ret = context.render();
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Chebyshev.prototype._getCoefficient = function(x, degree, memo) {
            if (memo.hasOwnProperty(degree)) return memo[degree];
            else if (degree === 0) memo[degree] = 0;
            else if (degree === 1) memo[degree] = x;
            else memo[degree] = 2 * x * this._getCoefficient(x, degree - 1, memo) - this._getCoefficient(x, degree - 2, memo);
            return memo[degree];
        };
        /**
		 ...
		 */ Object.defineProperty(Tone.Chebyshev.prototype, "order", {
            ...
            set: function(order) {
                ...
                for(var i = 0; i < len; ++i){
                    var x = i * 2 / len - 1;
                    if (x === 0) //should output 0 when input is 0
                    curve[i] = 0;
                    else curve[i] = this._getCoefficient(x, order, {});
                }
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Freeverb = function() {
            ...
            for(var c = 0; c < combFilterTunings.length; c++){
                ...
                if (c < combFilterTunings.length / 2) this.effectSendL.chain(lfpf, this._allpassFiltersL[0]);
                else this.effectSendR.chain(lfpf, this._allpassFiltersR[0]);
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.JCReverb = function() {
            ...
            for(var cf = 0; cf < combFilterDelayTimes.length; cf++){
                ...
                if (cf < combFilterDelayTimes.length / 2) fbcf.connect(this.effectReturnL);
                else fbcf.connect(this.effectReturnR);
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.BufferSource.prototype.getStateAtTime = function(time) {
            time = this.toSeconds(time);
            if (this._startTime !== -1 && time >= this._startTime && !this._sourceStopped) return Tone.State.Started;
            else return Tone.State.Stopped;
        };
        /**
		 *  Start the buffer
		 ...
		 */ Tone.BufferSource.prototype.start = function(time, offset, duration, gain, fadeInTime) {
            if (this._startTime !== -1) throw new Error("Tone.BufferSource can only be started once.");
            if (!this.buffer.loaded) throw new Error("Tone.BufferSource: buffer is either not set or not loaded.");
            time = this.toSeconds(time);
            //if it's a loop the default offset is the loopstart point
            if (this.loop) offset = Tone.defaultArg(offset, this.loopStart);
            ...
            if (fadeInTime > 0) {
                ...
                if (this.curve === "linear") this._gainNode.gain.linearRampToValueAtTime(this._gain, time + fadeInTime);
                else this._gainNode.gain.exponentialApproachValueAtTime(this._gain, time, fadeInTime);
            ...
            if (Tone.isDefined(duration)) {
                ...
                if (!this.loop) computedDur = Math.min(computedDur, this.buffer.duration - offset % this.buffer.duration);
            ...
            if (this.loop) {
                ...
                if (offset >= loopEnd) offset = (offset - loopStart) % loopDuration + loopStart;
            }
		 ...
		 */ Tone.BufferSource.prototype.stop = function(time, fadeOutTime) {
            ...
            if (this._stopTime !== -1) this.cancelStop();
            ...
            if (!this.loop) //make sure the fade does not go beyond the length of the buffer
            ...
            if (fadeOutTime > this.sampleTime) {
                ...
                if (this.curve === "linear") this._gainNode.gain.linearRampToValueAtTime(0, time);
                else this._gainNode.gain.exponentialApproachValueAtTime(0, startFade, fadeOutTime);
            } else this._gainNode.gain.setValueAtTime(0, time);
		 ...
		 */ Tone.BufferSource.prototype._onended = function() {
            if (!this._sourceStopped) {
                ...
                if (this._sourceStarted && this._stopTime !== -1) this._source.stop(this._stopTime + additionalTail);
                this.onended(this);
            }
        };
        /**
    ...
    Module(function(Tone) {
		 ...
		 */ Object.defineProperty(Tone.Noise.prototype, "type", {
            ...
            set: function(type) {
                if (this._type !== type) {
                    ...
                    } else throw new TypeError("Tone.Noise: invalid type: " + type);
                }
            }
        });
        /**
		 ...
		 */ var _noiseArrays = {
            ...
            "white": function() {
                ...
                for(var channelNum = 0; channelNum < channels; channelNum++){
                    var channel = new Float32Array(bufferLength);
                    buffer[channelNum] = channel;
                    for(var i = 0; i < bufferLength; i++)channel[i] = Math.random() * 2 - 1;
                }
                return buffer;
            }()
        };
        ...
        function createBuffers() {
            for(var type in _noiseArrays)_noiseBuffers[type] = new Tone.Buffer().fromArray(_noiseArrays[type]);
        }
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Event.prototype._rescheduleEvents = function(after) {
            ...
            this._state.forEachFrom(after, (function(event) {
                ...
                if (event.state === Tone.State.Started) {
                    if (Tone.isDefined(event.id)) Tone.Transport.clear(event.id);
                    var startTick = event.time + Math.round(this.startOffset / this._playbackRate);
                    if (this._loop) {
                        duration = Infinity;
                        if (Tone.isNumber(this._loop)) duration = this._loop * this._getLoopDuration();
                    ...
                    } else event.id = Tone.Transport.schedule(this._tick.bind(this), Tone.Ticks(startTick));
                }
            }).bind(this));
            return this;
        };
		 ...
		 */ Tone.Event.prototype._tick = function(time) {
            ...
            if (!this.mute && this._state.getValueAtTime(ticks) === Tone.State.Started) {
                ...
                if (this.humanize) {
                    var variation = 0.02;
                    if (!Tone.isBoolean(this.humanize)) variation = this.toSeconds(this.humanize);
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Part = function() {
			 ...
			 */ this._events = [];
            //add the events
            for(var i = 0; i < options.events.length; i++)if (Array.isArray(options.events[i])) this.add(options.events[i][0], options.events[i][1]);
            else this.add(options.events[i]);
        };
		 ...
		 */ Tone.Part.prototype.start = function(time, offset) {
            ...
            if (this._state.getValueAtTime(ticks) !== Tone.State.Started) {
                if (this._loop) offset = Tone.defaultArg(offset, this._loopStart);
                else offset = Tone.defaultArg(offset, 0);
		 ...
		 */ Tone.Part.prototype._startNote = function(event, ticks, offset) {
            ...
            } else if (event.startOffset >= offset) event.start(Tone.Ticks(ticks));
        };
        /**
		 *  The start from the scheduled start time
		 *  @type {Ticks}
		 ...
		 */ Tone.Part.prototype.at = function(time, value) {
            ...
            for(var i = 0; i < this._events.length; i++){
                ...
                if (Math.abs(time.toTicks() - event.startOffset) < tickTime) {
                    if (Tone.isDefined(value)) event.value = value;
                    return event;
                }
            }
		 ...
		 */ Tone.Part.prototype.remove = function(time, value) {
            ...
            for(var i = this._events.length - 1; i >= 0; i--){
                var event = this._events[i];
                if (event instanceof Tone.Part) event.remove(time, value);
		 ...
		 */ Tone.Part.prototype._forEach = function(callback, ctx) {
            if (this._events) {
                ...
                for(var i = this._events.length - 1; i >= 0; i--){
                    ...
                    if (e instanceof Tone.Part) e._forEach(callback, ctx);
                    else callback.call(ctx, e);
                }
            }
            return this;
        };
		 ...
		 */ Tone.Part.prototype._testLoopBoundries = function(event) {
            if (event.startOffset < this._loopStart || event.startOffset >= this._loopEnd) event.cancel(0);
            else if (event.state === Tone.State.Stopped) //reschedule it if it's stopped
            this._restartEvent(event);
        };
        /**
		 *  The probability of the notes being triggered.
		 ...
		 */ Object.defineProperty(Tone.Part.prototype, "loopEnd", {
            ...
            set: function(loopEnd) {
                this._loopEnd = this.toTicks(loopEnd);
                if (this._loop) this._forEach(function(event) {
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Sequence = function() {
            ...
            if (Tone.isUndef(options.loopEnd) && Tone.isDefined(events)) this._loopEnd = events.length * this._subdivision;
            //defaults to looping
            this._loop = true;
            //add all of the events
            if (Tone.isDefined(events)) for(var i = 0; i < events.length; i++)this.add(i, events[i]);
        };
		 ...
		 */ Tone.Sequence.prototype._indexTime = function(index) {
            if (index instanceof Tone.TransportTime) return index;
            else return Tone.Ticks(index * this._subdivision + this.startOffset).toSeconds();
        };
        /**
		 *  Clean up.
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.FatOscillator.prototype._forEach = function(iterator) {
            for(var i = 0; i < this._oscillators.length; i++)iterator.call(this, this._oscillators[i], i);
        };
        /**
		 * The type of the carrier oscillator
            ...
            set: function(count) {
                ...
                if (this._oscillators.length !== count) {
                    ...
                    for(var i = 0; i < count; i++){
                        ...
                        if (this.type === Tone.Oscillator.Type.Custom) osc1.partials = this._partials;
                        else osc1.type = this._type;
                        osc1.phase = this._phase;
                        osc1.volume.value = -6 - count * 1.1;
                        this.frequency.connect(osc1.frequency);
                        this.detune.connect(osc1.detune);
                        osc1.connect(this.output);
                        this._oscillators[i] = osc1;
                    }
                    //set the spread
                    this.spread = this._spread;
                    if (this.state === Tone.State.Started) this._forEach(function(osc) {
                        osc.start();
                    });
                }
            }
    ...
    Module(function(Tone) {
		 ...
		 */ Object.defineProperty(Tone.OmniOscillator.prototype, "type", {
            get: function() {
                var prefix = "";
                if (this._sourceType === OmniOscType.FM) prefix = "fm";
                else if (this._sourceType === OmniOscType.AM) prefix = "am";
                else if (this._sourceType === OmniOscType.Fat) prefix = "fat";
            ...
            set: function(type) {
                ...
                } else if (type === "pwm") this._createNewOscillator(OmniOscType.PWM);
                else if (type === "pulse") this._createNewOscillator(OmniOscType.Pulse);
                else {
		 ...
		 */ Tone.OmniOscillator.prototype.set = function(params, value) {
            ...
            if (params === "type") this.type = value;
            else if (Tone.isObject(params) && params.hasOwnProperty("type")) this.type = params.type;
            //then set the rest
		 ...
		 */ Tone.OmniOscillator.prototype._createNewOscillator = function(oscType) {
            if (oscType !== this._sourceType) {
                ...
                this.detune.connect(this._oscillator.detune);
                this._oscillator.connect(this.output);
                if (this.state === Tone.State.Started) this._oscillator.start(now);
            }
        };
        /**
		 * The phase of the oscillator in degrees.
		 ...
		 */ Object.defineProperty(Tone.OmniOscillator.prototype, "width", {
            get: function() {
                if (this._sourceType === OmniOscType.Pulse) return this._oscillator.width;
            }
        });
        /**
		 * The number of detuned oscillators
		 ...
		 */ Object.defineProperty(Tone.OmniOscillator.prototype, "count", {
            get: function() {
                if (this._sourceType === OmniOscType.Fat) return this._oscillator.count;
            },
            set: function(count) {
                if (this._sourceType === OmniOscType.Fat) this._oscillator.count = count;
            }
        });
        /**
		 * The detune spread between the oscillators. If "count" is
		 ...
		 */ Object.defineProperty(Tone.OmniOscillator.prototype, "spread", {
            get: function() {
                if (this._sourceType === OmniOscType.Fat) return this._oscillator.spread;
            },
            set: function(spread) {
                if (this._sourceType === OmniOscType.Fat) this._oscillator.spread = spread;
            }
        });
        /**
		 ...
		 */ Object.defineProperty(Tone.OmniOscillator.prototype, "modulationType", {
            get: function() {
                if (this._sourceType === OmniOscType.FM || this._sourceType === OmniOscType.AM) return this._oscillator.modulationType;
            },
            set: function(mType) {
                if (this._sourceType === OmniOscType.FM || this._sourceType === OmniOscType.AM) this._oscillator.modulationType = mType;
            }
        });
        /**
		 * The modulation index which is in essence the depth or amount of the modulation. In other terms it is the
		 ...
		 */ Object.defineProperty(Tone.OmniOscillator.prototype, "modulationIndex", {
            get: function() {
                if (this._sourceType === OmniOscType.FM) return this._oscillator.modulationIndex;
            }
        });
        /**
		 *  Harmonicity is the frequency ratio between the carrier and the modulator oscillators.
		 ...
		 */ Object.defineProperty(Tone.OmniOscillator.prototype, "harmonicity", {
            get: function() {
                if (this._sourceType === OmniOscType.FM || this._sourceType === OmniOscType.AM) return this._oscillator.harmonicity;
            }
        });
        /**
		 ...
		 */ Object.defineProperty(Tone.OmniOscillator.prototype, "modulationFrequency", {
            get: function() {
                if (this._sourceType === OmniOscType.PWM) return this._oscillator.modulationFrequency;
            }
        });
        /**
		 *  Clean up.
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Synth.prototype._triggerEnvelopeAttack = function(time, velocity) {
            ...
            if (this.envelope.sustain === 0) this.oscillator.stop(time + this.envelope.attack + this.envelope.decay);
            return this;
        };
        /**
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.MetalSynth.prototype.triggerAttack = function(time, vel) {
            ...
            if (this.envelope.sustain === 0) this._oscillators.forEach((function(osc) {
		 ...
		 */ Object.defineProperty(Tone.MetalSynth.prototype, "modulationIndex", {
            ...
            set: function(val) {
                for(var i = 0; i < this._oscillators.length; i++)this._oscillators[i].modulationIndex.value = val;
            }
        });
        /**
		 ...
		 */ Object.defineProperty(Tone.MetalSynth.prototype, "harmonicity", {
            ...
            set: function(val) {
                for(var i = 0; i < this._oscillators.length; i++)this._oscillators[i].harmonicity.value = val;
            }
        });
        /**
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.NoiseSynth.prototype.triggerAttack = function(time, velocity) {
            ...
            if (this.envelope.sustain === 0) this.noise.stop(time = this.envelope.attack + this.envelope.decay);
            return this;
        };
        /**
		 *  Start the release portion of the envelopes.
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.PolySynth = function() {
            ...
            for(var i = 0; i < options.polyphony; i++){
                ...
                if (!(v instanceof Tone.Monophonic)) throw new Error("Synth constructor must be instance of Tone.Monophonic");
                this.voices[i] = v;
                v.connect(this.output);
                if (v.hasOwnProperty("detune")) this.detune.connect(v.detune);
                this._triggers[i] = {
                    release: -1,
                    note: null,
                    voice: v
		 ...
		 */ Tone.PolySynth.prototype.triggerAttack = function(notes, time, velocity) {
            ...
            for(var i = 0; i < notes.length; i++){
                ...
                for(var j = 1; j < this._triggers.length; j++)if (this._triggers[j].release < oldest.release) oldest = this._triggers[j];
		 ...
		 */ Tone.PolySynth.prototype.triggerAttackRelease = function(notes, duration, time, velocity) {
            ...
            if (Tone.isArray(duration) && Tone.isArray(notes)) for(var i = 0; i < notes.length; i++){
		 ...
		 */ Tone.PolySynth.prototype.set = function(params, value, rampTime) {
            for(var i = 0; i < this.voices.length; i++)this.voices[i].set(params, value, rampTime);
            return this;
        };
        /**
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Sampler = function(urls) {
            ...
            for(var note in urls){
                ...
                } else if (!isNaN(parseFloat(note))) //otherwise if it's numbers assume it's midi
                urlMap[note] = urls[note];
                else throw new Error("Tone.Sampler: url keys must be the note's pitch");
            }
            /**
		 ...
		 */ Tone.Sampler.prototype.triggerAttack = function(note, time, velocity) {
            ...
            if (difference !== null) {
                ...
                if (!Tone.isArray(this._activeSources[midi])) this._activeSources[midi] = [];
		 ...
		 */ Tone.Sampler.prototype.add = function(note, url, callback) {
            ...
            else throw new Error("Tone.Sampler: note must be the note's pitch. Instead got " + note);
        };
        /**
		 * If the buffers are loaded or not
		 ...
		 */ Tone.Sampler.prototype.dispose = function() {
            ...
            this._buffers = null;
            for(var midi in this._activeSources)this._activeSources[midi].forEach(function(event) {
    ...
    Module(function(Tone) {
        if (Tone.supported) {
            if (!OscillatorNode.prototype.setPeriodicWave) OscillatorNode.prototype.setPeriodicWave = OscillatorNode.prototype.setWaveTable;
            if (!AudioContext.prototype.createPeriodicWave) AudioContext.prototype.createPeriodicWave = AudioContext.prototype.createWaveTable;
        }
    });
    Module(function(Tone) {
        /**
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Player = function(url) {
            ...
            } else options = Tone.defaults(arguments, [
                "url",
                "onload"
            ], Tone.Player);
            Tone.Source.call(this, options);
            ...
            if (url instanceof AudioBuffer) this._buffer.set(url);
            /**
			 *  if the buffer should loop once it's over
			 *  @type {Boolean}
			 *  @private
		 ...
		 */ Tone.Player.prototype._start = function(startTime, offset, duration) {
            ...
            this._activeSources.push(source);
            //start it
            if (this._loop && Tone.isUndef(duration)) source.start(startTime, offset);
            else source.start(startTime, offset, computedDuration);
            return this;
        };
        /**
		 *  Stop playback.
		 ...
		 */ Object.defineProperty(Tone.Player.prototype, "loop", {
            ...
            set: function(loop) {
                ...
                this._loop = loop;
                var now = this.now();
                if (!loop) //stop the playback on the next cycle
		 ...
		 */ Object.defineProperty(Tone.Player.prototype, "playbackRate", {
            ...
            set: function(rate) {
                ...
                //if it's not looping
                if (!this._loop) this._stopAtNextIteration(now);
                //set all the sources
                this._activeSources.forEach(function(source) {
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Players.prototype.get = function(name) {
            if (this.has(name)) return this._players[name];
            else throw new Error("Tone.Players: no player named " + name);
        };
        /**
		 ...
		 */ Tone.Players.prototype._forEach = function(callback) {
            for(var playerName in this._players)callback(this._players[playerName], playerName);
            return this;
        };
        /**
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.UserMedia.prototype.open = function(labelOrId) {
            return Tone.UserMedia.enumerateDevices().then((function(devices) {
                var device1;
                if (Tone.isNumber(labelOrId)) device1 = devices[labelOrId];
                else {
                    device1 = devices.find(function(device) {
                        return device.label === labelOrId || device.deviceId === labelOrId;
                    });
                    //didn't find a matching device
                    if (!device1 && devices.length > 0) device1 = devices[0];
                    else if (!device1 && Tone.isDefined(labelOrId)) throw new Error("Tone.UserMedia: no matching device: " + labelOrId);
                }
                this._device = device1;
                //do getUserMedia
                var constraints = {
                    audio: {
                ...
                if (device1) constraints.audio.deviceId = device1.deviceId;
    ...
    Module(function(Tone) {
		 ...
		 */ Tone.Midi = function(val, units) {
            if (this instanceof Tone.Midi) Tone.Frequency.call(this, val, units);
            else return new Tone.Midi(val, units);
        };
...
},{}],"ZNCMN":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _lookPath = require("./lookPath");
var _lookPathDefault = parcelHelpers.interopDefault(_lookPath);
...
var _shipBulletTypeDefault = parcelHelpers.interopDefault(_shipBulletType);
var _aniEffectMethod = require("./aniEffectMethod");
var _bricks = require("./bricks");
var _bricksDefault = parcelHelpers.interopDefault(_bricks);
var _sound = require("./sound");
var _shipSkillType = require("./shipSkillType");
var _shipSkillTypeDefault = parcelHelpers.interopDefault(_shipSkillType);
var _positionMethod = require("./positionMethod");
var _config = require("./config");
function bulletPosition() {
    ...
    for(var key in bulletArr){
        ...
        if (bulletArr[key].data.clear) bulletArr.splice(key, 1);
    }
}
function grapicBullet(viewDom) {
    ...
    thisBullet.map(function(el) {
        ...
        viewDom.drawImage(bulletImg, el.data.x - 6.5, el.data.y - 5, 13, 64);
        // defense
        enemyBulleArr.forEach(function(enemyBullet, index) {
            ...
            if (enemyBullet.data.position === ps + (0, _config.w)) {
                enemyBulleArr.splice(index, 1);
                thisBullet.splice(thisBullet.indexOf(el), 1);
            }
        });
        // kill enemy
        (0, _config.renderData).enemy.map(function(obj) {
            obj.wasHit(ps, viewDom, function() {
                thisBullet.splice(thisBullet.indexOf(el), 1);
            });
            obj.wasHit(ps + (0, _config.w), viewDom, function() {
                thisBullet.splice(thisBullet.indexOf(el), 1);
            });
        });
    });
...
function launchSkillsByShip(position, Type, ship) {
    var skillArr = (0, _config.renderData).shipSkills.push((0, _shipSkillTypeDefault.default)[Type](position, ship));
}
exports.default = function(obj) {
    this.name = obj.name; //名字
    this.life = obj.life; //生命值
    ...
    this.maxLife = obj.life;
    this.skills = obj.skills || [];
    this.path = (0, _lookPathDefault.default)[this.look](this.position, this.lookType).map((el)=>el.ps);
    this.pathObj = (0, _lookPathDefault.default)[this.look](this.position, this.lookType);
    this.invincible = false;
    var deadCb = obj.deadCb || function() {};
    var hit = false;
    this.wasHit = function(data) {
        var { bulletPs , bulletIndex , viewDom , shipPath  } = data;
        if (shipPath.includes(bulletPs) || shipPath.includes(bulletPs + (0, _config.w))) {
            this.injured();
            (0, _config.renderData).enemyBullet.splice(bulletIndex, 1);
        }
    };
    this.injured = function() {
        ...
        (0, _sound.deadSound)();
        this.life--;
        (0, _config.renderData).aniEffect.push((0, _aniEffectMethod.animation)(10, (function(renCount, viewDom) {
            this.pathObj.forEach(function(ps, index) {
                (0, _bricksDefault.default)(ps, viewDom, "rgba(255,0,0,.8)");
            });
        }).bind(this)));
        if (this.life < 1) {
            this.life = 0;
    ...
    this.touch = function(data) {
        ...
        if (touchEnemy) this.injured();
    };
    this.shot = function() {
        if (!this.isDead) {
            //
            (0, _shipBulletTypeDefault.default)[this.bulletType](this.position, this.bullet);
            //
            var shotPositon = this.position;
            (0, _config.renderData).aniEffect.push((0, _aniEffectMethod.animation)(30, function(renCount, viewDom) {
    ...
    this.grapic = function(viewDom) {
        ...
        var path = (0, _lookPathDefault.default)[this.look](this.position, this.lookType).map((el)=>el.ps);
        var pathObj = (0, _lookPathDefault.default)[this.look](this.position, this.lookType);
        this.path = path;
        this.pathObj = pathObj;
        bulletPosition.call(this);
        grapicBullet.call(this, viewDom);
        if (!isDead) {
            (0, _lookPathDefault.default)[this.look](this.position, this.lookType).forEach(function(ps, index) {
                (0, _bricksDefault.default)(ps, viewDom);
            });
            // touchEnemy
            var enemyMap = (0, _config.renderData).enemy.reduce(function(total, el1) {
                return [
                    ...total,
                    ...(0, _lookPathDefault.default)[el1.look](el1.position).map((el)=>el.ps)
                ];
            }, []);
            ...
            // touch item
            (0, _config.renderData).item.forEach(function(item, index) {
                var itemMap = (0, _lookPathDefault.default)[item.look](item.position).map(function(el) {
                    return el.ps;
                });
                var touchItem = path.reduce(function(total, el) {
                    return total || itemMap.includes(el);
                }, false);
                if (touchItem) //console.log(JSON.stringify(itemMap),JSON.stringify(path));
                item.wasPickUp(_);
            });
        }
    };
    this.skill = function(keycode) {
        var _this = this;
        var skill1 = this.skills.find((skill)=>skill.skillKey === keycode);
        skill1.skillPoint(this.position).forEach(function(position) {
            launchSkillsByShip(position, skill1.type, _this);
            isLaunchSkill = true;
            lookType = skill1.skillLook;
        });
    // this.grapic(viewDom, lookType);
    };
    this.dead = function() {
...
},{"./lookPath":"zDHOl","./movePathList":"bWHoT","./shipBulletType":"c69N4","./aniEffectMethod":"k9PxR","./bricks":"fh8xc","./sound":"kdkJ4","./shipSkillType":"35D1r","./positionMethod":"gqbqK","./config":"4Wc5b","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"c69N4":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _config = require("./config");
var _positionMethod = require("./positionMethod");
...
exports.default = {
    ...
    treble: function(bulletPs, arr, time) {
        normal(bulletPs, arr);
        if (bulletPs % (0, _config.w) > 1) normal(bulletPs - 1, arr);
        if (bulletPs % (0, _config.w) < (0, _config.w) - 1) normal(bulletPs + 1, arr);
    }
};

},{"./config":"4Wc5b","./positionMethod":"gqbqK","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"35D1r":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _config = require("./config");
var _positionMethod = require("./positionMethod");
...
exports.default = {
    atomicExplosion: function(position, ship) {
        ...
        return function(ship, viewDom) {
            ...
            skillPath.forEach(function(ps) {
                //var color = count % 2 ? 'rgba(255,0,0,.5)' : 'rgba(255,0,0,1)';
                var color = "rgba(" + Math.round(255 - 200 / time * (time - count)) + ",80," + Math.round(50 + 205 / time * (time - count)) + "," + (count / time * 0.5 + 0.5).toFixed(1) + ")";
                //bricks(ps, viewDom, 'blue');
                if (ps.ps) {
                    viewDom.fillStyle = color;
                    viewDom.fillRect(ps.x, ps.y, 15, 15);
    ...
    atomicExplosionCanKill: function(position, ship1) {
        var time = 300 - (40 - (0, _config.w)) * 20;
        var prePs = ship1.position;
        var count = time;
        return function(ship, viewDom) {
            count--;
            var t = 1 / time * (time - count);
...
},{"./config":"4Wc5b","./positionMethod":"gqbqK","./skillPath":"lttCW","./bricks":"fh8xc","./aniEffectMethod":"k9PxR","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"gkVl1":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _lookPath = require("./lookPath");
var _lookPathDefault = parcelHelpers.interopDefault(_lookPath);
...
var _bricks = require("./bricks");
var _bricksDefault = parcelHelpers.interopDefault(_bricks);
function restoreLifeAction(type, value1, ship1) {
    ...
    var actionMap = {
        ...
        liftRestore: function(ship, value) {
            var val = parseInt(value);
            if (ship.life + val > ship.maxLife) return ship.life = ship.maxLife;
            return ship.life += val;
        }
    };
    if (actionMap[type]) actionMap[type](ship1, value1);
}
function itemMove() {
    if ((0, _movePathListDefault.default)[this.movePath] && this.getST() % this.moveTime == 0) (0, _movePathListDefault.default)[this.movePath].call(this);
}
exports.default = function(obj) {
    ...
    this.wasPickUp = function(ship) {
        (0, _config.renderData).item.splice((0, _config.renderData).item.indexOf(this), 1);
        var position = this.position;
        var deadPs = (0, _positionMethod.positionToXY)(position);
        var restoreLife = this.restoreLife;
        var changeBullet = this.changeBullet;
        // restoreLife
        if (restoreLife.length > 0) restoreLife.forEach(function(val) {
            var action = val.split("-");
            restoreLifeAction(action[0], action[1], ship);
        });
        // changeBullet
        if (changeBullet) ship.bulletType = changeBullet;
        effect(ship);
        delete this;
    };
};

},{"./lookPath":"zDHOl","./movePathList":"bWHoT","./aniEffectMethod":"k9PxR","./positionMethod":"gqbqK","./config":"4Wc5b","./bricks":"fh8xc","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["dB9nQ","bB7Pu"], "bB7Pu", "parcelRequiree69f")
